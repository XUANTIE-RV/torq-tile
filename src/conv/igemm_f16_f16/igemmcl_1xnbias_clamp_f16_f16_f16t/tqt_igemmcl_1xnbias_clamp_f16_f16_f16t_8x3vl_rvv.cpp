...
#include "tqt_igemmcl_1xnbias_clamp_f16_f16_f16t_8x3vl_rvv.h"
...
// CL output [spatial, OC] is contiguous in OC — standard vle16/vse16.
...
static void igemmcl_kernel(size_t IC, size_t IC_stride, size_t kd_start, size_t kd_end, size_t KH,
                           size_t KW, float16_t *D, const float16_t *W,
                           const float16_t *input_group, const float16_t *C, const float16_t *bias,
                           size_t ldd, size_t ldw, size_t ldc, float16_t clamp_min,
                           float16_t clamp_max, size_t vl, const int *od_arr, const int *oh_arr,
                           const int *ow_arr, size_t ID, size_t IH, size_t IW, size_t SD, size_t SH,
        ...
        for (size_t kh = 0; kh < KH; kh++) {
            ...
            for (size_t kw = 0; kw < KW; kw++) {
                ...
                for (size_t ic = 0; ic < IC; ic++) {
                    ...
                    tqt_load_bt_kernel_gemm_f16_f16_8x3vl_rvv<NR>(vl, w_ptr, ldw);
                    // FMA
...
static void igemmcl_dispatch(size_t M, size_t N, size_t IC, size_t IC_stride, size_t kd_start,
                             size_t kd_end, size_t KH, size_t KW, float16_t *D, const float16_t *W,
                             const float16_t *input_group, const float16_t *C,
                             const float16_t *bias, size_t ldd, size_t ldw, size_t ldc,
    ...
    const void *bias, float16_t clamp_min, float16_t clamp_max, const tqt_conv_params *params)
{
    ...
    const float16_t *input_group =
        (const float16_t *)input + group_idx * IC_per_group;  // CL: IC is last dim
    ...
    for (size_t m_idx_local = 0; m_idx_local < m; m_idx_local += m_step) {
        ...
        for (size_t n_idx = 0; n_idx < n; n_idx += n_step) {
            ...
            const float16_t *w_ptr = (const float16_t *)B + n_idx * ldb + k_idx_b;
