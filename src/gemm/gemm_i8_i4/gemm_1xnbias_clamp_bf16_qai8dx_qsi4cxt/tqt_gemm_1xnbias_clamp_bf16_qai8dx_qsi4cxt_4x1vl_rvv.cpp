...
#include "tqt_gemm_1xnbias_clamp_bf16_qai8dx_qsi4cxt_4x1vl_rvv.h"
...
size_t tqt_get_m_step_gemm_1xnbias_clamp_bf16_qai8dx_qsi4cxt_4x1vl_rvv(void)
{
    return m_step;
}

size_t tqt_get_n_step_gemm_1xnbias_clamp_bf16_qai8dx_qsi4cxt_4x1vl_rvv(void)
{
    return n_step;
}

size_t tqt_get_a_offset_gemm_1xnbias_clamp_bf16_qai8dx_qsi4cxt_4x1vl_rvv(size_t m_idx, size_t k_idx,
                                                                         size_t lda)
{
...
size_t tqt_get_b_offset_gemm_1xnbias_clamp_bf16_qai8dx_qsi4cxt_4x1vl_rvv(size_t n_idx, size_t k_idx,
                                                                         size_t ldb)
{
...
size_t tqt_get_c_offset_gemm_1xnbias_clamp_bf16_qai8dx_qsi4cxt_4x1vl_rvv(size_t m_idx, size_t n_idx,
                                                                         size_t ldc)
{
    TQT_ASSUME(m_idx % m_step == 0);
    TQT_ASSUME(n_idx % n_step == 0);
...
size_t tqt_get_bias_offset_gemm_1xnbias_clamp_bf16_qai8dx_qsi4cxt_4x1vl_rvv(size_t n_idx)
{
    TQT_ASSUME(n_idx % n_step == 0);
    return n_idx * sizeof(bfloat16_t);
}

size_t tqt_get_d_offset_gemm_1xnbias_clamp_bf16_qai8dx_qsi4cxt_4x1vl_rvv(size_t m_idx, size_t n_idx,
                                                                         size_t ldd)
{
    TQT_ASSUME(m_idx % m_step == 0);
    TQT_ASSUME(n_idx % n_step == 0);
...
size_t tqt_get_d_size_gemm_1xnbias_clamp_bf16_qai8dx_qsi4cxt_4x1vl_rvv(size_t m, size_t n)
{
...
template <size_t MR>
static void gemm_kernel(size_t K, bfloat16_t *D, const int8_t *A, const uint8_t *B,
                        ...
                        size_t vl, const float *scale_a, const int32_t *zp_a, const float *scale_b)
{
    ...
    if (C) {
        tqt_add_c_bf16_kernel_gemm_i8_i4_4x1vl_rvv<MR>(vl, C, c_stride_row);
    }

    if (bias) {
        tqt_add_1xnbias_bf16_kernel_gemm_i8_i4_4x1vl_rvv<MR>(vl, bias);
    }
    ...
    tqt_store_narrow_bf16_kernel_gemm_i8_i4_4x1vl_rvv<MR>(vl, D, d_stride_row);
}

using gemm_kernel_fn_t = void (*)(size_t K, bfloat16_t *D, const int8_t *A, const uint8_t *B,
                                  const bfloat16_t *C, const bfloat16_t *bias, size_t ldd,
                                  size_t lda, size_t ldb, size_t ldc, bfloat16_t clamp_min,
                                  bfloat16_t clamp_max, size_t vl, const float *scale_a,
...
void tqt_run_gemm_1xnbias_clamp_bf16_qai8dx_qsi4cxt_4x1vl_rvv(
    ...
    bfloat16_t clamp_min, bfloat16_t clamp_max, const struct tqt_qai8dx_qsi4cx_params *params)
{
    TQT_UNUSED(k_idx_a);
    TQT_UNUSED(k_idx_b);
    ...
    const bfloat16_t *C_ptr = (const bfloat16_t *)C;
    const bfloat16_t *bias_ptr = (const bfloat16_t *)bias;
    bfloat16_t *D_ptr = (bfloat16_t *)D;
    ...
    for (size_t m_idx = 0; m_idx < m; m_idx += m_step) {
        for (size_t n_idx = 0; n_idx < n; n_idx += n_step) {
            ...
            const bfloat16_t *c_tile = C_ptr ? C_ptr + m_idx * ldc + n_idx : nullptr;
            const bfloat16_t *bias_tile = bias_ptr ? bias_ptr + n_idx : nullptr;
            bfloat16_t *d_tile = D_ptr + m_idx * ldd + n_idx;
