...
#include "tqt_gemm_1xnbias_clamp_f16_f16p_f16p_8x3vl_rvv.h"
#include "tqt_gemm_1xnbias_clamp_f16_f16p_f16p_interface.h"

namespace
{

/// Micro-kernel interface
const tqt_gemm_1xnbias_clamp_f16_f16p_f16p_ukernel ukernel{
    ...
    for (size_t i = 0; i < m; ++i) {
        for (size_t j = 0; j < n; ++j) {
            ...
            for (size_t p = 0; p < k; ++p) {
                ...
                float16_t b_val = B[j * ldb + p];  // B is transposed
            ...
            if (bias != nullptr) {
                acc = static_cast<float16_t>(static_cast<float>(acc) + static_cast<float>(bias[j]));
            }
...
int main()
{
    ...
    float16_t *B = (float16_t *)malloc(N * K * sizeof(float16_t));  // B is n x k (transposed)
    ...
    ukernel.run_bt_pack(N, K, K, K, 0, B, B_packed);  // Use bt_pack for transposed B
    ...
    printf("TEST[gemm_1xnbias_clamp_f16_f16p_f16p_pack_bt]\n");
    printf("- ukernel: gemm_1xnbias_clamp_f16_f16p_f16p_8x3vl_rvv\n");
