...
#include "tqt_gemm_1xnbias_clamp_f16_f16p_f16p_4x4vl_rvv.h"
...
size_t tqt_get_m_step_gemm_1xnbias_clamp_f16_f16p_f16p_4x4vl_rvv(void)
{
    return m_step;
}

size_t tqt_get_n_step_gemm_1xnbias_clamp_f16_f16p_f16p_4x4vl_rvv(void)
{
    return n_step;
}

size_t tqt_get_a_packed_offset_gemm_1xnbias_clamp_f16_f16p_f16p_4x4vl_rvv(size_t m_idx,
...
size_t tqt_get_b_packed_offset_gemm_1xnbias_clamp_f16_f16p_f16p_4x4vl_rvv(size_t n_idx,
...
size_t tqt_get_c_offset_gemm_1xnbias_clamp_f16_f16p_f16p_4x4vl_rvv(size_t m_idx, size_t n_idx,
                                                                   size_t ldc)
{
    TQT_ASSUME(m_idx % m_step == 0);
    TQT_ASSUME(n_idx % n_step == 0);
...
size_t tqt_get_bias_offset_gemm_1xnbias_clamp_f16_f16p_f16p_4x4vl_rvv(size_t n_idx)
{
    TQT_ASSUME(n_idx % n_step == 0);
    return n_idx * sizeof(float16_t);
}

size_t tqt_get_d_offset_gemm_1xnbias_clamp_f16_f16p_f16p_4x4vl_rvv(size_t m_idx, size_t n_idx,
                                                                   size_t ldd)
{
    TQT_ASSUME(m_idx % m_step == 0);
    TQT_ASSUME(n_idx % n_step == 0);
...
size_t tqt_get_d_size_gemm_1xnbias_clamp_f16_f16p_f16p_4x4vl_rvv(size_t m, size_t n)
{
    return m * n * sizeof(float16_t);
}

size_t tqt_get_a_packed_size_gemm_1xnbias_clamp_f16_f16p_f16p_4x4vl_rvv(size_t m, size_t k)
{
    return m * k * sizeof(float16_t);
}

size_t tqt_get_b_packed_size_gemm_1xnbias_clamp_f16_f16p_f16p_4x4vl_rvv(size_t n, size_t k)
{
...
void tqt_run_a_pack_gemm_1xnbias_clamp_f16_f16p_f16p_4x4vl_rvv(size_t m, size_t k, size_t lda,
...
void tqt_run_b_pack_gemm_1xnbias_clamp_f16_f16p_f16p_4x4vl_rvv(size_t n, size_t k, size_t ldb,
                                                               ...
                                                               const void *B, void *B_packed)
{
    ...
    // region (per-k row stride = actual_n_in_block).
    ...
    for (size_t n_idx = 0; n_idx < n; n_idx += n_step) {
        ...
        // is element-size-agnostic but operates at LMUL=1, so when actual_n
        // exceeds vlmax we issue multiple calls covering disjoint column ranges
        // of the same logical [k, actual_n] block. Passing n=actual_n makes the
        // helper use actual_n as its per-k destination row stride.
...
void tqt_run_bt_pack_gemm_1xnbias_clamp_f16_f16p_f16p_4x4vl_rvv(size_t n, size_t k, size_t ldb,
                                                                size_t ldb_packed, size_t k_idx,
                                                                const void *B, void *B_packed)
{
    // Pack transposed B [n, k] into the same flat [k, actual_n_in_block] per-block layout.
    // Inner loop slices by vlmax because the nxk e16 helper kernel is LMUL=1.
                            ...
                            size_t vl)
{
    ...
    // Prologue
    tqt_load_a_kernel_gemm_f16_f16_4x4vl_rvv<MR>(a_k_ptr, a_stride_row);
    tqt_load_b_kernel_gemm_f16_f16_4x4vl_rvv(vl, b_k_ptr);
    a_k_ptr += MR;
    b_k_ptr += vl;

    // Steady-state K-loop
    for (size_t k_idx = 1; k_idx < K; k_idx++) {
        tqt_fused_load_vfmacc_axb_kernel_gemm_f16_f16_4x4vl_rvv<MR>(vl, a_k_ptr, a_stride_row,
                                                                    b_k_ptr);
        a_k_ptr += MR;
        b_k_ptr += vl;
    }
...
void tqt_run_gemm_1xnbias_clamp_f16_f16p_f16p_4x4vl_rvv(size_t m, size_t n, size_t k, const void *A,
