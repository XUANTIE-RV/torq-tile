...
#include "tqt_gemm_1xnbias_clamp_bf16_qai8dx_qsi4cxt_interface.h"

namespace
{

const tqt_gemm_1xnbias_clamp_bf16_qai8dx_qsi4cxt_ukernel ukernel{
    tqt_get_m_step_gemm_1xnbias_clamp_bf16_qai8dx_qsi4cxt_4x1vl_rvv,
    tqt_get_n_step_gemm_1xnbias_clamp_bf16_qai8dx_qsi4cxt_4x1vl_rvv,
    tqt_get_a_offset_gemm_1xnbias_clamp_bf16_qai8dx_qsi4cxt_4x1vl_rvv,
    tqt_get_b_offset_gemm_1xnbias_clamp_bf16_qai8dx_qsi4cxt_4x1vl_rvv,
    tqt_get_c_offset_gemm_1xnbias_clamp_bf16_qai8dx_qsi4cxt_4x1vl_rvv,
    tqt_get_bias_offset_gemm_1xnbias_clamp_bf16_qai8dx_qsi4cxt_4x1vl_rvv,
    tqt_get_d_offset_gemm_1xnbias_clamp_bf16_qai8dx_qsi4cxt_4x1vl_rvv,
    tqt_get_d_size_gemm_1xnbias_clamp_bf16_qai8dx_qsi4cxt_4x1vl_rvv,
    tqt_run_gemm_1xnbias_clamp_bf16_qai8dx_qsi4cxt_4x1vl_rvv};
...
int main()
{
    ...
    // bf16 has the same exponent range as fp32 (~1e38), so we can use a wider
    // clamp than fp16 here without overflow. Picked 1e30 for headroom.
    const float clamp_min_f = -1.0e30f;
    const float clamp_max_f = 1.0e30f;
    ...
    memset(D, 0, M * N * sizeof(bfloat16_t));
    ...
    // bf16 has 7-bit mantissa (vs 10-bit for fp16) so cosine threshold is
    // slightly looser; W4A8 quantization noise dominates either way.
    const bool passed = cs > 0.99f;

    printf("TEST[gemm_1xnbias_clamp_bf16_qai8dx_qsi4cxt]\n");
    printf("- ukernel: gemm_1xnbias_clamp_bf16_qai8dx_qsi4cxt_4x1vl_rvv\n");
