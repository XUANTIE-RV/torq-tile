...
///
/// Reused from tqt_kernel_gemm_f16_f16_8x3vl_rvv.h (call with swapped <NR, MR>):
...
///   vlse16.v v{reg}, (base), stride_c * sizeof(float16_t)
template <size_t MR, size_t NR>
static inline void tqt_init_c_strided_kernel_igemmcf_f16_f16_3vlx8_rvv(size_t vl,
                                                                       const float16_t *C,
                                                                       size_t stride_c,
                                                                       size_t vlmax)
{
    ...
    size_t stride_bytes = stride_c * sizeof(float16_t);
    asm volatile("vsetvli zero, %0, e16, m1, ta, ma\n\t" : : "r"(vl) :);

    // nr=0: v8, v9, v10
    if constexpr (NR >= 1) {
        const float16_t *c_nr_0 = C + 0;
        if constexpr (MR >= 1)
    ...
    // nr=1: v11, v12, v13
    if constexpr (NR >= 2) {
        const float16_t *c_nr_1 = C + 1;
        if constexpr (MR >= 1)
            asm volatile("vlse16.v v11, (%0), %1"
                         :
                         : "r"(c_nr_1 + 0 * vlmax * stride_c), "r"(stride_bytes)
                         : "v11", "memory");
        if constexpr (MR >= 2)
            asm volatile("vlse16.v v12, (%0), %1"
                         :
                         : "r"(c_nr_1 + 1 * vlmax * stride_c), "r"(stride_bytes)
                         : "v12", "memory");
        if constexpr (MR >= 3)
            asm volatile("vlse16.v v13, (%0), %1"
                         :
    ...
    // nr=2: v14, v15, v16
    if constexpr (NR >= 3) {
        const float16_t *c_nr_2 = C + 2;
        if constexpr (MR >= 1)
            asm volatile("vlse16.v v14, (%0), %1"
                         :
                         : "r"(c_nr_2 + 0 * vlmax * stride_c), "r"(stride_bytes)
                         : "v14", "memory");
        if constexpr (MR >= 2)
            asm volatile("vlse16.v v15, (%0), %1"
                         :
                         : "r"(c_nr_2 + 1 * vlmax * stride_c), "r"(stride_bytes)
                         : "v15", "memory");
        if constexpr (MR >= 3)
            asm volatile("vlse16.v v16, (%0), %1"
                         :
    ...
    // nr=3: v17, v18, v19
    if constexpr (NR >= 4) {
        const float16_t *c_nr_3 = C + 3;
        if constexpr (MR >= 1)
            asm volatile("vlse16.v v17, (%0), %1"
                         :
                         : "r"(c_nr_3 + 0 * vlmax * stride_c), "r"(stride_bytes)
                         : "v17", "memory");
        if constexpr (MR >= 2)
            asm volatile("vlse16.v v18, (%0), %1"
                         :
                         : "r"(c_nr_3 + 1 * vlmax * stride_c), "r"(stride_bytes)
                         : "v18", "memory");
        if constexpr (MR >= 3)
            asm volatile("vlse16.v v19, (%0), %1"
                         :
    ...
    // nr=4: v20, v21, v22
    if constexpr (NR >= 5) {
        const float16_t *c_nr_4 = C + 4;
        if constexpr (MR >= 1)
            asm volatile("vlse16.v v20, (%0), %1"
                         :
                         : "r"(c_nr_4 + 0 * vlmax * stride_c), "r"(stride_bytes)
                         : "v20", "memory");
        if constexpr (MR >= 2)
            asm volatile("vlse16.v v21, (%0), %1"
                         :
                         : "r"(c_nr_4 + 1 * vlmax * stride_c), "r"(stride_bytes)
                         : "v21", "memory");
        if constexpr (MR >= 3)
            asm volatile("vlse16.v v22, (%0), %1"
                         :
    ...
    // nr=5: v23, v24, v25
    if constexpr (NR >= 6) {
        const float16_t *c_nr_5 = C + 5;
        if constexpr (MR >= 1)
            asm volatile("vlse16.v v23, (%0), %1"
                         :
                         : "r"(c_nr_5 + 0 * vlmax * stride_c), "r"(stride_bytes)
                         : "v23", "memory");
        if constexpr (MR >= 2)
            asm volatile("vlse16.v v24, (%0), %1"
                         :
                         : "r"(c_nr_5 + 1 * vlmax * stride_c), "r"(stride_bytes)
                         : "v24", "memory");
        if constexpr (MR >= 3)
            asm volatile("vlse16.v v25, (%0), %1"
                         :
    ...
    // nr=6: v26, v27, v28
    if constexpr (NR >= 7) {
        const float16_t *c_nr_6 = C + 6;
        if constexpr (MR >= 1)
            asm volatile("vlse16.v v26, (%0), %1"
                         :
                         : "r"(c_nr_6 + 0 * vlmax * stride_c), "r"(stride_bytes)
                         : "v26", "memory");
        if constexpr (MR >= 2)
            asm volatile("vlse16.v v27, (%0), %1"
                         :
                         : "r"(c_nr_6 + 1 * vlmax * stride_c), "r"(stride_bytes)
                         : "v27", "memory");
        if constexpr (MR >= 3)
            asm volatile("vlse16.v v28, (%0), %1"
                         :
    ...
    // nr=7: v29, v30, v31
    if constexpr (NR >= 8) {
        const float16_t *c_nr_7 = C + 7;
        if constexpr (MR >= 1)
            asm volatile("vlse16.v v29, (%0), %1"
                         :
                         : "r"(c_nr_7 + 0 * vlmax * stride_c), "r"(stride_bytes)
                         : "v29", "memory");
        if constexpr (MR >= 2)
            asm volatile("vlse16.v v30, (%0), %1"
                         :
                         : "r"(c_nr_7 + 1 * vlmax * stride_c), "r"(stride_bytes)
                         : "v30", "memory");
        if constexpr (MR >= 3)
            asm volatile("vlse16.v v31, (%0), %1"
                         :
...
///   vsse16.v v{reg}, (base), stride_d * sizeof(float16_t)
template <size_t MR, size_t NR>
static inline void tqt_store_strided_kernel_igemmcf_f16_f16_3vlx8_rvv(size_t vl, float16_t *D,
                                                                      size_t stride_d, size_t vlmax)
{
    static_assert(MR >= 1 && MR <= 3, "MR must be in range [1, 3]");
    static_assert(NR >= 1 && NR <= 8, "NR must be in range [1, 8]");

    size_t stride_bytes = stride_d * sizeof(float16_t);
    asm volatile("vsetvli zero, %0, e16, m1, ta, ma\n\t" : : "r"(vl) :);

    // nr=0: v8, v9, v10
    if constexpr (NR >= 1) {
        float16_t *d_nr_0 = D + 0;
        if constexpr (MR >= 1)
            asm volatile("vsse16.v v8, (%0), %1"
                         :
                         : "r"(d_nr_0 + 0 * vlmax * stride_d), "r"(stride_bytes)
                         : "memory");
        if constexpr (MR >= 2)
            asm volatile("vsse16.v v9, (%0), %1"
                         :
                         : "r"(d_nr_0 + 1 * vlmax * stride_d), "r"(stride_bytes)
                         : "memory");
        if constexpr (MR >= 3)
            asm volatile("vsse16.v v10, (%0), %1"
                         :
    ...
    // nr=1: v11, v12, v13
    if constexpr (NR >= 2) {
        float16_t *d_nr_1 = D + 1;
        if constexpr (MR >= 1)
            asm volatile("vsse16.v v11, (%0), %1"
                         :
                         : "r"(d_nr_1 + 0 * vlmax * stride_d), "r"(stride_bytes)
                         : "memory");
        if constexpr (MR >= 2)
            asm volatile("vsse16.v v12, (%0), %1"
                         :
                         : "r"(d_nr_1 + 1 * vlmax * stride_d), "r"(stride_bytes)
                         : "memory");
        if constexpr (MR >= 3)
            asm volatile("vsse16.v v13, (%0), %1"
                         :
    ...
    // nr=2: v14, v15, v16
    if constexpr (NR >= 3) {
        float16_t *d_nr_2 = D + 2;
        if constexpr (MR >= 1)
            asm volatile("vsse16.v v14, (%0), %1"
                         :
                         : "r"(d_nr_2 + 0 * vlmax * stride_d), "r"(stride_bytes)
                         : "memory");
        if constexpr (MR >= 2)
            asm volatile("vsse16.v v15, (%0), %1"
                         :
                         : "r"(d_nr_2 + 1 * vlmax * stride_d), "r"(stride_bytes)
                         : "memory");
        if constexpr (MR >= 3)
            asm volatile("vsse16.v v16, (%0), %1"
                         :
    ...
    // nr=3: v17, v18, v19
    if constexpr (NR >= 4) {
        float16_t *d_nr_3 = D + 3;
        if constexpr (MR >= 1)
            asm volatile("vsse16.v v17, (%0), %1"
                         :
                         : "r"(d_nr_3 + 0 * vlmax * stride_d), "r"(stride_bytes)
                         : "memory");
        if constexpr (MR >= 2)
            asm volatile("vsse16.v v18, (%0), %1"
                         :
                         : "r"(d_nr_3 + 1 * vlmax * stride_d), "r"(stride_bytes)
                         : "memory");
        if constexpr (MR >= 3)
            asm volatile("vsse16.v v19, (%0), %1"
                         :
    ...
    // nr=4: v20, v21, v22
    if constexpr (NR >= 5) {
        float16_t *d_nr_4 = D + 4;
        if constexpr (MR >= 1)
            asm volatile("vsse16.v v20, (%0), %1"
                         :
                         : "r"(d_nr_4 + 0 * vlmax * stride_d), "r"(stride_bytes)
                         : "memory");
        if constexpr (MR >= 2)
            asm volatile("vsse16.v v21, (%0), %1"
                         :
                         : "r"(d_nr_4 + 1 * vlmax * stride_d), "r"(stride_bytes)
                         : "memory");
        if constexpr (MR >= 3)
            asm volatile("vsse16.v v22, (%0), %1"
                         :
    ...
    // nr=5: v23, v24, v25
    if constexpr (NR >= 6) {
        float16_t *d_nr_5 = D + 5;
        if constexpr (MR >= 1)
            asm volatile("vsse16.v v23, (%0), %1"
                         :
                         : "r"(d_nr_5 + 0 * vlmax * stride_d), "r"(stride_bytes)
                         : "memory");
        if constexpr (MR >= 2)
            asm volatile("vsse16.v v24, (%0), %1"
                         :
                         : "r"(d_nr_5 + 1 * vlmax * stride_d), "r"(stride_bytes)
                         : "memory");
        if constexpr (MR >= 3)
            asm volatile("vsse16.v v25, (%0), %1"
                         :
    ...
    // nr=6: v26, v27, v28
    if constexpr (NR >= 7) {
        float16_t *d_nr_6 = D + 6;
        if constexpr (MR >= 1)
            asm volatile("vsse16.v v26, (%0), %1"
                         :
                         : "r"(d_nr_6 + 0 * vlmax * stride_d), "r"(stride_bytes)
                         : "memory");
        if constexpr (MR >= 2)
            asm volatile("vsse16.v v27, (%0), %1"
                         :
                         : "r"(d_nr_6 + 1 * vlmax * stride_d), "r"(stride_bytes)
                         : "memory");
        if constexpr (MR >= 3)
            asm volatile("vsse16.v v28, (%0), %1"
                         :
    ...
    // nr=7: v29, v30, v31
    if constexpr (NR >= 8) {
        float16_t *d_nr_7 = D + 7;
        if constexpr (MR >= 1)
            asm volatile("vsse16.v v29, (%0), %1"
                         :
                         : "r"(d_nr_7 + 0 * vlmax * stride_d), "r"(stride_bytes)
                         : "memory");
        if constexpr (MR >= 2)
            asm volatile("vsse16.v v30, (%0), %1"
                         :
                         : "r"(d_nr_7 + 1 * vlmax * stride_d), "r"(stride_bytes)
                         : "memory");
        if constexpr (MR >= 3)
            asm volatile("vsse16.v v31, (%0), %1"
                         :
