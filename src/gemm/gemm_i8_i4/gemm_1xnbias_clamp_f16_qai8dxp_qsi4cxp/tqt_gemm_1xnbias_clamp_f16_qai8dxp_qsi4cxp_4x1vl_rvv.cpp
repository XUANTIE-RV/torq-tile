...
#include "tqt_gemm_1xnbias_clamp_f16_qai8dxp_qsi4cxp_4x1vl_rvv.h"
...
size_t tqt_get_m_step_gemm_1xnbias_clamp_f16_qai8dxp_qsi4cxp_4x1vl_rvv(void)
{
    return m_step;
}

size_t tqt_get_n_step_gemm_1xnbias_clamp_f16_qai8dxp_qsi4cxp_4x1vl_rvv(void)
{
    return n_step;
}

size_t tqt_get_a_packed_offset_gemm_1xnbias_clamp_f16_qai8dxp_qsi4cxp_4x1vl_rvv(size_t m_idx,
                                                                                size_t k_idx,
                                                                                size_t lda,
                                                                                size_t actual_m)
{
...
size_t tqt_get_b_packed_offset_gemm_1xnbias_clamp_f16_qai8dxp_qsi4cxp_4x1vl_rvv(size_t n_idx,
                                                                                size_t k_idx,
                                                                                size_t ldb,
                                                                                size_t actual_n)
{
...
size_t tqt_get_c_offset_gemm_1xnbias_clamp_f16_qai8dxp_qsi4cxp_4x1vl_rvv(size_t m_idx, size_t n_idx,
                                                                         size_t ldc)
{
    TQT_ASSUME(m_idx % m_step == 0);
    TQT_ASSUME(n_idx % n_step == 0);
...
size_t tqt_get_bias_offset_gemm_1xnbias_clamp_f16_qai8dxp_qsi4cxp_4x1vl_rvv(size_t n_idx)
{
    TQT_ASSUME(n_idx % n_step == 0);
    return n_idx * sizeof(float16_t);
}

size_t tqt_get_d_offset_gemm_1xnbias_clamp_f16_qai8dxp_qsi4cxp_4x1vl_rvv(size_t m_idx, size_t n_idx,
                                                                         size_t ldd)
{
    TQT_ASSUME(m_idx % m_step == 0);
    TQT_ASSUME(n_idx % n_step == 0);
...
size_t tqt_get_d_size_gemm_1xnbias_clamp_f16_qai8dxp_qsi4cxp_4x1vl_rvv(size_t m, size_t n)
{
    return m * n * sizeof(float16_t);
}

size_t tqt_get_a_packed_size_gemm_1xnbias_clamp_f16_qai8dxp_qsi4cxp_4x1vl_rvv(size_t m, size_t k)
{
    return m * k * sizeof(int8_t);
}

size_t tqt_get_b_packed_size_gemm_1xnbias_clamp_f16_qai8dxp_qsi4cxp_4x1vl_rvv(size_t n, size_t k)
{
...
void tqt_run_a_pack_gemm_1xnbias_clamp_f16_qai8dxp_qsi4cxp_4x1vl_rvv(size_t m, size_t k, size_t lda,
                                                                     size_t lda_packed,
...
void tqt_run_bt_pack_gemm_1xnbias_clamp_f16_qai8dxp_qsi4cxp_4x1vl_rvv(size_t n, size_t k,
                                                                      size_t ldb, size_t ldb_packed,
                                                                      size_t k_idx, const void *B,
                                                                      void *B_packed)
{
    // B is int4 packed as [n, k/2] uint8 with k int4 columns. Pack treating
    // each pair of nibbles as 1 byte: [n, k/2] u8 -> [n/vl, k/2, vl] u8.
...
template <size_t MR>
static void gemm_kernel(size_t K, float16_t *D, const int8_t *A_packed, const uint8_t *B_packed,
                        ...
                        const float *scale_a, const int32_t *zp_a, const float *scale_b)
{
    ...
    // Packed A: [m_step, k] interleaved. For the k-th column, A_packed[k * MR + mi].
    // Packed B: [k/2, n_step] for this N tile. For the kp-th k-pair,
    //           B_packed + kp * vl. Each byte holds 2 K values.
    for (size_t k = 0; k < K; k += 2) {
        ...
        // Interleaved A: A_packed[k * MR + mi] for k+0; A_packed[(k+1) * MR + mi] for k+1.
...
using gemm_kernel_fn_t = void (*)(size_t K, float16_t *D, const int8_t *A_packed,
                                  const uint8_t *B_packed, const float16_t *C,
...
void tqt_run_gemm_1xnbias_clamp_f16_qai8dxp_qsi4cxp_4x1vl_rvv(
    ...
    const struct tqt_qai8dx_qsi4cx_params *params)
{
    ...
    for (size_t m_idx = 0; m_idx < m; m_idx += m_step) {
        for (size_t n_idx = 0; n_idx < n; n_idx += n_step) {
            ...
            // Packed A tile starts at m_idx * k (interleaved per m-tile).
            const int8_t *a_tile = A_ptr + m_idx * k;
            // Packed B tile starts at n_idx * (k/2).
