...
#include "tqt_convcf_mx1bias_clamp_f16_f16p_f16p_interface.h"

namespace
{
/// Micro-kernel interface
const tqt_convcf_mx1bias_clamp_f16_f16p_f16p_ukernel ukernel{
    tqt_get_m_step_convcf_mx1bias_clamp_f16_f16p_f16p_8x3vl_rvv,
    tqt_get_n_step_convcf_mx1bias_clamp_f16_f16p_f16p_8x3vl_rvv,
    tqt_get_a_packed_offset_convcf_mx1bias_clamp_f16_f16p_f16p_8x3vl_rvv,
    tqt_get_b_im2colpack_offset_convcf_mx1bias_clamp_f16_f16p_f16p_8x3vl_rvv,
    tqt_get_c_offset_convcf_mx1bias_clamp_f16_f16p_f16p_8x3vl_rvv,
    tqt_get_bias_offset_convcf_mx1bias_clamp_f16_f16p_f16p_8x3vl_rvv,
    tqt_get_d_offset_convcf_mx1bias_clamp_f16_f16p_f16p_8x3vl_rvv,
    tqt_get_d_size_convcf_mx1bias_clamp_f16_f16p_f16p_8x3vl_rvv,
...
void reference_conv_cf(const float16_t *activation, const float16_t *weight, const float16_t *bias,
                            ...
                            for (size_t kh = 0; kh < KH; ++kh) {
                                for (size_t kw = 0; kw < KW; ++kw) {
                                    ...
                                    if (id >= 0 && id < (int)ID && ih >= 0 && ih < (int)IH &&
                                        ...
                                        acc +=
                                            static_cast<float>(
                                                activation[ic * ID * IH * IW +
                                                           (size_t)id * IH * IW + (size_t)ih * IW +
                                                           (size_t)iw]) *
                                            static_cast<float>(weight[oc * IC_pg * KD * KH * KW +
                                                                      ic * KD * KH * KW +
                                                                      kd * KH * KW + kh * KW + kw]);
                                    }
                                }
...
int main()
{
    ...
    // Allocate the memory
    float16_t *activation = (float16_t *)malloc(IC_pg * ID * IH * IW * sizeof(float16_t));
    ...
    reference_conv_cf(activation, weight, bias, ref_output, &params, cmin, cmax);

    // Run micro-kernel implementation using ukernel interface
    // Pack A (weight) [M, K]
    size_t a_packed_size = ukernel.get_a_packed_size(M, K);
    void *A_packed = malloc(a_packed_size);
    ukernel.run_a_pack(M, K, K, K, 0, weight, A_packed);

    // Fused im2col+pack B (activation) -> B_packed
    size_t b_packed_size = ukernel.get_b_im2colpack_size(N, K);
    void *B_packed = malloc(b_packed_size);
    ukernel.run_b_im2colpack(0, activation, K, 0, B_packed, &params);
    ...
    printf("TEST[convcf_mx1bias_clamp_f16_f16p_f16p]\n");
    printf("- ukernel: convcf_mx1bias_clamp_f16_f16p_f16p_8x3vl_rvv\n");
