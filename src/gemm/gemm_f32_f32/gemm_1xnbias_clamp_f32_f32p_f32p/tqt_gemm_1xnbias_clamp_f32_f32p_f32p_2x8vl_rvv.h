...
/// Micro-kernel: GEMM with 1xN bias and clamp for FP32 (A and B packed)
///
/// This is a rvv (2 x 8vl tile) implementation using LMUL=8 configuration.
/// vl = csrr_vlenb() / sizeof(float), n_step = vl * 8.
/// Computes: D = clamp(C + A_packed*B_packed + bias, min, max)
/// where A and B are in packed format for optimal memory access.

/// Gets m step value.
///
/// The starting row index must be divisible by `m_step`.
///
/// @return The m step value (2).
...
///
/// @return The n step value (8 * vl, where vl = csrr_vlenb() / sizeof(float)).
...
/// @param[in]  k_idx Starting k index for packing.
/// @param[in]  B Input matrix B [n, k] in row-major format.
...
///
void tqt_run_gemm_1xnbias_clamp_f32_f32p_f32p_2x8vl_rvv(
