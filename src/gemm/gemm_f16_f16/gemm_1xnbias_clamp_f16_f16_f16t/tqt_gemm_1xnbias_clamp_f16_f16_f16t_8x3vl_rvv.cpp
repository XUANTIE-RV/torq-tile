                        ...
                        size_t ldb, size_t ldc, float16_t clamp_min, float16_t clamp_max, size_t vl)
{
    ...
    tqt_load_bt_kernel_gemm_f16_f16_8x3vl_rvv<NR>(vl, b_ptr, b_stride_row);
    a_ptr += 1;
    b_ptr += 1;

    // Steady-state K-loop
    for (size_t k_idx = 1; k_idx < K; k_idx++) {
        tqt_fused_load_vfmacc_axbt_kernel_gemm_f16_f16_8x3vl_rvv<MR, NR>(vl, a_ptr, a_stride_row,
                                                                         b_ptr, b_stride_row);
        a_ptr += 1;
        b_ptr += 1;
    }
