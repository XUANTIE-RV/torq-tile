...
#include "tqt_dwconvcl_1xnbias_clamp_f16_f16_f16t_interface.h"

namespace
{
const tqt_dwconvcl_1xnbias_clamp_f16_f16_f16t_ukernel ukernel{
...
// Reference dwconv (NDHWC). bias may be NULL.
void reference_dwconv_cl(const float16_t *input, const float16_t *weight, const float16_t *bias,
        ...
        for (size_t od = 0; od < OD; ++od)
            for (size_t oh = 0; oh < OH; ++oh)
                for (size_t ow = 0; ow < OW; ++ow) {
                        ...
                        for (size_t kh = 0; kh < KH; ++kh)
                            for (size_t kw = 0; kw < KW; ++kw) {
                                ...
                                if (id < 0 || id >= (int)ID || ih < 0 || ih >= (int)IH || iw < 0 ||
                                    iw >= (int)IW)
                                    continue;
                                acc += input[((size_t)id * IH * IW + (size_t)ih * IW + (size_t)iw) *
                                                 IC +
                                             ic] *
                    ...
                    if (acc > cmax)
                        acc = cmax;
                    output[((size_t)od * OH * OW + oh * OW + ow) * OC + oc] = acc;
                }
    }
}
...
int main(void)
{
    const size_t IC = 8, OC = 8, IH = 16, IW = 16, KH = 3, KW = 3;
    tqt_conv_params p;
    p.input_shape[0] = 1;
    p.input_shape[1] = IH;
    p.input_shape[2] = IW;
    ...
    reference_dwconv_cl(input, weight, bias, out_ref, &p, (float16_t)0.0f, FLT_MAX);
    ukernel.run_dwconv(input, weight, bias, out_actual, &p, (float16_t)0.0f, FLT_MAX);
    ...
    std::free(out_ref);
    std::free(out_actual);
    std::printf("TEST[dwconvcl_1xnbias_clamp_f16_f16_f16t]\n");
    std::printf("- ukernel: dwconvcl_1xnbias_clamp_f16_f16_f16t_1x1vl_rvv\n");
