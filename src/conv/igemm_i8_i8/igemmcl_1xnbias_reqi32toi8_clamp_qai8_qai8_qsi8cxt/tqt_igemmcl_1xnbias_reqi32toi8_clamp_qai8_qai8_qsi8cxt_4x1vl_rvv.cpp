...
#include "tqt_igemmcl_1xnbias_reqi32toi8_clamp_qai8_qai8_qsi8cxt_4x1vl_rvv.h"
...
// ============================================================================
// Internal: Template-based igemmcl kernel (transposed weight, int8)
// ============================================================================

// Direct reuse of GEMM i32_i8_i8 4x1vl kernel functions.
// igemmcl vectorization direction (N=OC) matches GEMM N direction.
// CL output [spatial, OC] is contiguous in OC -> standard vle8/vse8.
// NO K-tiling: full K always processed, no is_first_k/is_last_k branching.
template <size_t MR>
static void igemmcl_kernel(size_t IC, size_t IC_stride, size_t KD, size_t KH, size_t KW, int8_t *D,
                           const int8_t *W, const int8_t *input_group, const int32_t *bias,
                           size_t ldd, size_t ldw, int8_t clamp_min, int8_t clamp_max, size_t vl,
                           const int *od_arr, const int *oh_arr, const int *ow_arr, size_t ID,
                           size_t IH, size_t IW, size_t SD, size_t SH, size_t SW, size_t DD,
                           ...
                           const int32_t *mult, const int32_t *shift, int32_t zp)
{
        ...
        for (size_t kh = 0; kh < KH; kh++) {
            ...
            for (size_t kw = 0; kw < KW; kw++) {
                ...
                for (size_t ic = 0; ic < IC; ic++) {
                    ...
                    // Load weight vector (transposed B, strided) into v0
    ...
    // Post: bias + requantize + clamp + store (always, no K-tiling branches)
    if (bias) {
...
using igemmcl_kernel_fn_t = void (*)(size_t IC, size_t IC_stride, size_t KD, size_t KH, size_t KW,
                                     int8_t *D, const int8_t *W, const int8_t *input_group,
                                     const int32_t *bias, size_t ldd, size_t ldw, int8_t clamp_min,
...
static constexpr igemmcl_kernel_fn_t kernel_table[4] = {
    igemmcl_kernel<1>,
    igemmcl_kernel<2>,
    igemmcl_kernel<3>,
    igemmcl_kernel<4>,
...
static void igemmcl_dispatch(size_t M, size_t IC, size_t IC_stride, size_t KD, size_t KH, size_t KW,
                             int8_t *D, const int8_t *W, const int8_t *input_group,
                             const int32_t *bias, size_t ldd, size_t ldw, int8_t clamp_min,
                             ...
                             int32_t zp)
{
    ...
    kernel_table[mr_idx](IC, IC_stride, KD, KH, KW, D, W, input_group, bias, ldd, ldw, clamp_min,
                         clamp_max, vl, od_arr, oh_arr, ow_arr, ID, IH, IW, SD, SH, SW, DD, DH, DW,
                         PD, PH, PW, mult, shift, zp);
}

// ============================================================================
// Igemm Main Function (channel-last, transposed weight, int8)
// ============================================================================
    ...
    const struct tqt_qai8_qai8_qsi8cx_params *quant_params)
{
    ...
    for (size_t m_idx_local = 0; m_idx_local < m; m_idx_local += m_step) {
        ...
        for (size_t n_idx = 0; n_idx < n; n_idx += n_step) {
            ...
            // Weight pointer: B[n_idx, 0] in [OC, K] layout
            const int8_t *w_ptr = (const int8_t *)B + n_idx * ldb;
            ...
            igemmcl_dispatch(actual_m, IC_per_group, conv_params->input_channel, KD, KH, KW, d_ptr,
                             w_ptr, input_group, bias_ptr, ldd, ldb, clamp_min, clamp_max, actual_n,
                             od_arr, oh_arr, ow_arr, ID, IH, IW, SD, SH, SW, DD, DH, DW, PD, PH, PW,
