...
#include "tqt_dwconvcf_mx1bias_clamp_f16_f16_f16_1x1vl_rvv.h"
...
size_t tqt_get_output_size_dwconvcf_mx1bias_clamp_f16_f16_f16_1x1vl_rvv(
...
void tqt_run_dwconv_dwconvcf_mx1bias_clamp_f16_f16_f16_1x1vl_rvv(
    ...
    const tqt_conv_params *params, float16_t clamp_min, float16_t clamp_max)
{
    ...
    const float16_t *bias_base = (const float16_t *)bias;  // may be NULL
    ...
    for (size_t oc = 0; oc < OC; ++oc) {
        ...
        const float16_t *in_ic = in_base + ic * in_spatial;
        const float16_t *wt_oc = wt_base + oc * kernel_size;
        float16_t *out_oc = out_base + oc * out_spatial;
        const float16_t bias_val = (bias_base != NULL) ? bias_base[oc] : (float16_t)0.0f;

        for (size_t od = 0; od < OD; ++od) {
            for (size_t oh = 0; oh < OH; ++oh) {
                size_t ow = 0;
                while (ow < OW) {
                    const size_t vl = __riscv_vsetvl_e16m1(OW - ow);
                    vfloat16m1_t acc = __riscv_vfmv_v_f_f16m1(bias_val, vl);
                    ...
                    for (size_t kd = 0; kd < KD; ++kd) {
                        ...
                        for (size_t kh = 0; kh < KH; ++kh) {
                            ...
                            for (size_t kw = 0; kw < KW; ++kw) {
                                const float16_t w_val = wt_oc[kd * KH * KW + kh * KW + kw];
                                ...
                                if (iw_first >= 0 && iw_last < (long)IW) {
                                    ...
                                    const float16_t *src = in_ic + (size_t)id_in * IH * IW +
                                                           (size_t)ih_in * IW + (size_t)iw_first;
                                    vfloat16m1_t v_in = __riscv_vlse16_v_f16m1(
                                        src, (ptrdiff_t)(SW * sizeof(float16_t)), vl);
                                    acc = __riscv_vfmacc_vf_f16m1(acc, w_val, v_in, vl);
                                } else {
                                    ...
                                    vfloat16m1_t v_in = __riscv_vle16_v_f16m1(tmp, vl);
                                    acc = __riscv_vfmacc_vf_f16m1(acc, w_val, v_in, vl);
                                }
                            }
                        }
                    ...
                    __riscv_vse16_v_f16m1(out_oc + od * OH * OW + oh * OW + ow, acc, vl);
