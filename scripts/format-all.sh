...
FILES=$(find "$PROJECT_ROOT/src" "$PROJECT_ROOT/examples" "$PROJECT_ROOT/test" -type d \( -name "build" -o -name "CMakeFiles" \) -prune -o -type f \( -name "*.h" -o -name "*.hpp" -o -name "*.c" -o -name "*.cpp" \) -print 2>/dev/null)
