...
/// Micro-kernel: GEMM with Mx1 bias, requantization (int32->int8), and clamp
/// for qsi8dx (A per-channel) x qai8 (B) -> qai8 (D) with packed A and B
///
...
///
/// Quantization scheme:
/// - A: symmetric per-channel quantization (scale_a[m], quant_channel_a = M)
/// - B: asymmetric quantization (scale_b, zero_point_b)
...
size_t tqt_get_d_size_gemm_mx1bias_reqi32toi8_clamp_qai8_qsi8dxp_qai8p_4x1vl_rvv(size_t m,
                                                                                 size_t n);
...
/// Runs the GEMM microkernel with packed matrices, Mx1 bias, requantization, and clamp.
///
...
/// @param[in]  ldd Leading dimension of D.
/// @param[in]  bias Bias vector [m, 1] (int32), each row has its own bias.
...
/// @note The bias vector has shape [m, 1] and each row of the result gets its own bias.
