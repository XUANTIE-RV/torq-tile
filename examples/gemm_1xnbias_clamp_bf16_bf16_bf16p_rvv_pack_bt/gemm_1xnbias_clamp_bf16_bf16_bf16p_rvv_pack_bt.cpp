...
#include "tqt_gemm_1xnbias_clamp_bf16_bf16_bf16p_interface.h"

namespace
{

/// Micro-kernel interface (B-only packed: A original, B packed)
const tqt_gemm_1xnbias_clamp_bf16_bf16_bf16p_ukernel ukernel{
    ...
    for (size_t i = 0; i < m; ++i) {
        for (size_t j = 0; j < n; ++j) {
            ...
            if (acc < (float)clamp_min)
                ...
                acc = (float)clamp_max;
            D[i * ldd + j] = (bfloat16_t)acc;
        }
    }
}

void fill_matrix_random(size_t rows, size_t cols, bfloat16_t *matrix, float min, float max)
{
    for (size_t i = 0; i < rows * cols; ++i) {
        matrix[i] = (bfloat16_t)(min + (max - min) * ((float)rand() / (float)RAND_MAX));
    }
}
...
int main()
{
    ...
    bfloat16_t *B = (bfloat16_t *)malloc(N * K * sizeof(bfloat16_t));
    ...
    bfloat16_t clamp_min = (bfloat16_t)(-60000.0f), clamp_max = (bfloat16_t)(60000.0f);
    ...
    printf("TEST[gemm_1xnbias_clamp_bf16_bf16_bf16p_pack_bt]\n");
    printf("- ukernel: gemm_1xnbias_clamp_bf16_bf16_bf16p_8x1vl_rvv\n");
