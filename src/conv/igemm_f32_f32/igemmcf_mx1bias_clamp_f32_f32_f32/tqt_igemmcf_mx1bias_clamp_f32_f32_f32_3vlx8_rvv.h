...
/// Implicit GEMM micro-kernel: channel-first, Mx1 bias, clamp, non-packed weight, FP32
/// RVV 3vlx8 tile implementation.
///
/// Vectorization direction: M (OC) vectorized (3*vl), N (spatial) scalar (8).
/// A = weight [OC_per_group, K], input = [IC, ID, IH, IW] (fetched on-the-fly)
...
/// Gets m step value.
///
/// @return The m step value (3 * vl, where vl = csrr_vlenb() / sizeof(float)).
