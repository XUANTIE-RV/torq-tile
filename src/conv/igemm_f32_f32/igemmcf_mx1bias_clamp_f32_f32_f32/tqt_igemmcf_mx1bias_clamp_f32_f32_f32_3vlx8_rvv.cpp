...
#include "tqt_igemmcf_mx1bias_clamp_f32_f32_f32_3vlx8_rvv.h"
...
size_t tqt_get_m_step_igemmcf_mx1bias_clamp_f32_f32_f32_3vlx8_rvv(void)
{
    return m_step;
}

size_t tqt_get_n_step_igemmcf_mx1bias_clamp_f32_f32_f32_3vlx8_rvv(void)
{
    return n_step;
}

size_t tqt_get_a_offset_igemmcf_mx1bias_clamp_f32_f32_f32_3vlx8_rvv(size_t m_idx, size_t k_idx,
                                                                    size_t lda)
{
...
size_t tqt_get_c_offset_igemmcf_mx1bias_clamp_f32_f32_f32_3vlx8_rvv(size_t m_idx, size_t n_idx,
                                                                    size_t ldc)
{
    TQT_ASSUME(m_idx % m_step == 0);
...
size_t tqt_get_bias_offset_igemmcf_mx1bias_clamp_f32_f32_f32_3vlx8_rvv(size_t m_idx)
{
    TQT_ASSUME(m_idx % m_step == 0);
    return m_idx * sizeof(float);
}

size_t tqt_get_d_offset_igemmcf_mx1bias_clamp_f32_f32_f32_3vlx8_rvv(size_t m_idx, size_t n_idx,
                                                                    size_t ldd)
{
    TQT_ASSUME(m_idx % m_step == 0);
...
size_t tqt_get_d_size_igemmcf_mx1bias_clamp_f32_f32_f32_3vlx8_rvv(size_t m, size_t n)
{
...
static void igemmcf_kernel(size_t K_inner, size_t ic_start, size_t ic_end, float *D, const float *W,
                           const float *input_group, const float *C, const float *bias, size_t ldd,
                           size_t ldw, size_t ldc, float clamp_min, float clamp_max, size_t vl,
        ...
        for (size_t kd = 0; kd < KD; kd++) {
            ...
            for (size_t kh = 0; kh < KH; kh++) {
                ...
                for (size_t kw = 0; kw < KW; kw++) {
                    ...
                    tqt_load_bt_kernel_gemm_f32_f32_8x3vl_rvv<MR>(vl, w_ptr, ldw);
...
using igemmcf_kernel_fn_t = void (*)(size_t K_inner, size_t ic_start, size_t ic_end, float *D,
                                     const float *W, const float *input_group, const float *C,
                                     const float *bias, size_t ldd, size_t ldw, size_t ldc,
                                     float clamp_min, float clamp_max, size_t vl, const int *od_arr,
                                     const int *oh_arr, const int *ow_arr, size_t ID, size_t IH,
                                     size_t IW, size_t KD, size_t KH, size_t KW, size_t SD,
                                     size_t SH, size_t SW, size_t DD, size_t DH, size_t DW,
                                     size_t PD, size_t PH, size_t PW, bool is_first_k,
                                     bool is_last_k);
...
static void igemmcf_dispatch(size_t M, size_t N, size_t K_inner, size_t ic_start, size_t ic_end,
                             float *D, const float *W, const float *input_group, const float *C,
                             const float *bias, size_t ldd, size_t ldw, size_t ldc, float clamp_min,
                             ...
                             size_t DH, size_t DW, size_t PD, size_t PH, size_t PW, bool is_first_k,
                             bool is_last_k)
{
...
void tqt_run_igemm_igemmcf_mx1bias_clamp_f32_f32_f32_3vlx8_rvv(
    ...
    const void *bias, float clamp_min, float clamp_max, const tqt_conv_params *params)
{
    ...
    for (size_t n_idx_local = 0; n_idx_local < n; n_idx_local += n_step) {
        ...
        for (size_t m_idx = 0; m_idx < m; m_idx += m_step) {
            ...
            const float *w_ptr = (const float *)A + m_idx * lda + k_idx_a;
