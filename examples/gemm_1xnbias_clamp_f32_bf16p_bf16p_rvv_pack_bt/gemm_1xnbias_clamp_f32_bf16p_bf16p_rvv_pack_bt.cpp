...
#include "tqt_gemm_1xnbias_clamp_f32_bf16p_bf16p_interface.h"

namespace
{
const tqt_gemm_1xnbias_clamp_f32_bf16p_bf16p_ukernel ukernel{
...
// Reference implementation: B is transposed (stored as n x k)
    ...
    for (size_t i = 0; i < m; ++i) {
        for (size_t j = 0; j < n; ++j) {
            ...
            for (size_t p = 0; p < k; ++p) {
                acc += static_cast<float>(A[i * lda + p]) * static_cast<float>(B[j * ldb + p]);
            }
            if (C)
...
int main()
{
    ...
    bfloat16_t *B = (bfloat16_t *)malloc(N * K * sizeof(bfloat16_t));  // n x k (transposed)
    ...
    printf("TEST[gemm_1xnbias_clamp_f32_bf16p_bf16p_pack_bt]\n");
    printf("- ukernel: gemm_1xnbias_clamp_f32_bf16p_bf16p_8x1vl_rvv\n");
