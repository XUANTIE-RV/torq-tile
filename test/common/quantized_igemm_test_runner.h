...
struct NonPackedQuantizedIgemmcfFuncs
{
    ...
    using RunIgemmFunc = void (*)(size_t m, size_t n, size_t k, const void *input, size_t group_idx,
                                  ...
                                  const tqt_conv_params *conv_params, const QParams *quant_params);
...
template <typename QParams>
struct PackedQuantizedIgemmcfFuncs
{
    using GetMStepFunc = size_t (*)(void);
    using GetNStepFunc = size_t (*)(void);
...
template <typename QParams>
struct NonPackedQuantizedIgemmclFuncs
{
    using GetMStepFunc = size_t (*)(void);
    using GetNStepFunc = size_t (*)(void);
    using RunIgemmFunc = void (*)(size_t m, size_t n, size_t k, const void *input, size_t group_idx,
...
template <typename QParams>
struct PackedQuantizedIgemmclFuncs
{
    using GetMStepFunc = size_t (*)(void);
    using GetNStepFunc = size_t (*)(void);
...
// ============================================================================
// Non-packed igemmcf test runner: requantize int32 -> int8
// CF layout: A = weight [OC_pg, K], input = [IC, ID, IH, IW] (fetched on-the-fly)
// Mx1 bias pattern: A per-channel symmetric, B(input) global asymmetric
// ============================================================================

inline void run_nonpacked_igemmcf_reqi32toi8_test(
    const ConvTestParams &test_params,
    const NonPackedQuantizedIgemmcfFuncs<tqt_qai8_qsi8dx_qai8_params> &funcs)
{
    ...
    // Execute igemm kernel (no im2col needed, fetched on-the-fly)
    const int8_t clamp_min = -128;
    const int8_t clamp_max = 127;
    std::vector<int8_t> D_i8(M * N, 0);

    funcs.run_igemm(M, N, K, activation_i8.data(), 0, 0, 0, weight_i8.data(), K, 0, nullptr, N,
                    D_i8.data(), N, bias_fused.data(), clamp_min, clamp_max, &params, &qparams);
    ...
    EXPECT_TRUE(result.passed) << result.error_message
                               << "\n  Igemmcf NonPacked ReqI32toI8: " << test_params.name
...
// ============================================================================
// Packed igemmcf test runner: requantize int32 -> int8
// CF layout: A = weight [OC_pg, K] packed, input = [IC, ID, IH, IW] (fetched on-the-fly)
// Mx1 bias pattern: A per-channel symmetric, B(input) global asymmetric
// ============================================================================

inline void run_packed_igemmcf_reqi32toi8_test(
    const ConvTestParams &test_params,
    const PackedQuantizedIgemmcfFuncs<tqt_qai8_qsi8dx_qai8_params> &funcs)
{
    tqt_conv_params params = test_params.to_conv_params();
    ...
    // Execute igemm kernel (input fetched on-the-fly, no im2col)
    const int8_t clamp_min = -128;
    const int8_t clamp_max = 127;
    std::vector<int8_t> D_i8(M * N, 0);

    funcs.run_igemm(M, N, K, activation_i8.data(), 0, 0, 0, A_packed.data(), K, 0, nullptr, N,
                    D_i8.data(), N, bias_fused.data(), clamp_min, clamp_max, &params, &qparams);
    ...
    auto result = verify_gemm_result<float>(D_ref_fp32.data(), D_act_fp32.data(), M * N, tolerance,
                                            cosine_threshold);
    EXPECT_TRUE(result.passed) << result.error_message
                               << "\n  Igemmcf Packed ReqI32toI8: " << test_params.name
                               << "\n  IC=" << IC << " OC=" << params.output_channel
                               << "\n  Max abs error: " << result.max_abs_error
                               << "\n  Cosine similarity: " << result.cosine_similarity;
}
...
// ============================================================================
// Non-packed igemmcl test runner: requantize int32 -> int8
// CL layout: input = [ID, IH, IW, IC] (fetched on-the-fly), B = weight [N, K] (transposed)
// 1xN bias pattern: A(input) global asymmetric, B per-channel symmetric
// ============================================================================

inline void run_nonpacked_igemmcl_reqi32toi8_test(
    const ConvTestParams &test_params,
    const NonPackedQuantizedIgemmclFuncs<tqt_qai8_qai8_qsi8cx_params> &funcs)
{
    tqt_conv_params params = test_params.to_conv_params();
    ...
    auto result = verify_gemm_result<float>(D_ref_fp32.data(), D_act_fp32.data(), M * N, tolerance,
                                            cosine_threshold);
    EXPECT_TRUE(result.passed) << result.error_message
                               << "\n  Igemmcl NonPacked ReqI32toI8: " << test_params.name
                               << "\n  IC=" << IC << " OC=" << params.output_channel
                               << "\n  Max abs error: " << result.max_abs_error
                               << "\n  Cosine similarity: " << result.cosine_similarity;
}
...
// ============================================================================
// Packed igemmcl test runner: requantize int32 -> int8
// CL layout: input = [ID, IH, IW, IC] (fetched on-the-fly), B = weight [N, K] packed
// 1xN bias pattern: A(input) global asymmetric, B per-channel symmetric
// ============================================================================

inline void run_packed_igemmcl_reqi32toi8_test(
    const ConvTestParams &test_params,
    const PackedQuantizedIgemmclFuncs<tqt_qai8_qai8_qsi8cx_params> &funcs)
{
    tqt_conv_params params = test_params.to_conv_params();
    ...
    funcs.run_igemm(M, N, K, activation_i8.data(), 0, 0, 0, B_packed.data(), K, 0, nullptr, N,
                    D_i8.data(), N, bias_fused.data(), clamp_min, clamp_max, &params, &qparams);
    ...
    auto result = verify_gemm_result<float>(D_ref_fp32.data(), D_act_fp32.data(), M * N, tolerance,
                                            cosine_threshold);
    EXPECT_TRUE(result.passed) << result.error_message
                               << "\n  Igemmcl Packed ReqI32toI8: " << test_params.name
                               << "\n  IC=" << IC << " OC=" << params.output_channel
                               << "\n  Max abs error: " << result.max_abs_error
                               << "\n  Cosine similarity: " << result.cosine_similarity;
}
