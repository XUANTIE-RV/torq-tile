...
#include "tqt_gemm_1xnbias_clamp_f16_f16_f16_interface.h"

namespace
{
/// Micro-kernel interface
const tqt_gemm_1xnbias_clamp_f16_f16_f16_ukernel ukernel{
    ...
    for (size_t i = 0; i < m; ++i) {
        for (size_t j = 0; j < n; ++j) {
            ...
            for (size_t p = 0; p < k; ++p) {
                ...
                float16_t b_val = B[p * ldb + j];  // B is k x n
...
void fill_matrix(size_t rows, size_t cols, float16_t *matrix, const float weight)
{
    for (size_t i = 0; i < rows * cols; ++i) {
        matrix[i] = (float16_t)(i * weight);
    }
}
...
int main()
{
    ...
    float16_t *B = (float16_t *)malloc(K * N * sizeof(float16_t));  // B is k x n
    ...
    printf("TEST[gemm_1xnbias_clamp_f16_f16_f16]\n");
    printf("- ukernel: gemm_1xnbias_clamp_f16_f16_f16_8x3vl_rvv\n");
