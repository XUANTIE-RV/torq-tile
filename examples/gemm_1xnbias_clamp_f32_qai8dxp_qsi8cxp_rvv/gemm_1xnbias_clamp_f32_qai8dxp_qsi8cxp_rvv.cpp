...
#include "tqt_gemm_1xnbias_clamp_f32_qai8dxp_qsi8cxp_interface.h"

namespace
{

const tqt_gemm_1xnbias_clamp_f32_qai8dxp_qsi8cxp_ukernel ukernel{
    ...
    tqt_get_c_offset_gemm_1xnbias_clamp_f32_qai8dxp_qsi8cxp_4x1vl_rvv,
    ...
    tqt_run_b_pack_gemm_1xnbias_clamp_f32_qai8dxp_qsi8cxp_4x1vl_rvv,
                    ...
                    const float *act)
{
    bool passed = true;
    for (size_t i = 0; i < rows * cols; ++i) {
        if (fabsf(ref[i] - act[i]) > tolerance) {
            passed = false;
        }
    }
    return passed;
...
int main()
{
    ...
    float *B_fp32 = (float *)malloc(N * K * sizeof(float));  // B is [N, K] (transposed view)
    ...
    // Pack A and B
    int8_t *A_packed = (int8_t *)malloc(ukernel.get_a_packed_size(M, K));
    int8_t *B_packed = (int8_t *)malloc(ukernel.get_b_packed_size(N, K));
    float *D_act_fp32 = (float *)malloc(M * N * sizeof(float));

    if (!scale_a || !zp_a || !scale_b || !A_i8 || !BT_i8 || !A_packed || !B_packed || !D_act_fp32) {
        fprintf(stderr, "Memory allocation failed\n");
        return 1;
    }

    ukernel.run_a_pack(M, K, K, K, 0, A_i8, A_packed);
    // run_bt_pack expects B in [N, K] layout (already what BT_i8 is)
    ukernel.run_bt_pack(N, K, K, K, 0, BT_i8, B_packed);
    ...
    memset(D_act_fp32, 0, M * N * sizeof(float));
    ...
    for (size_t m_idx = 0; m_idx < M; m_idx += m_step) {
        for (size_t n_idx = 0; n_idx < N; n_idx += n_step) {
            ...
            const uint8_t *bias_ptr = (const uint8_t *)bias_fp32 + ukernel.get_bias_offset(n_idx);
            uint8_t *d_ptr = (uint8_t *)D_act_fp32 + ukernel.get_d_offset(m_idx, n_idx, N);
    ...
    const bool passed = passed_abs && (cosine_sim > 0.999f);

    printf("TEST[gemm_1xnbias_clamp_f32_qai8dxp_qsi8cxp_rvv]\n");
    printf("- ukernel: gemm_1xnbias_clamp_f32_qai8dxp_qsi8cxp_4x1vl_rvv\n");
