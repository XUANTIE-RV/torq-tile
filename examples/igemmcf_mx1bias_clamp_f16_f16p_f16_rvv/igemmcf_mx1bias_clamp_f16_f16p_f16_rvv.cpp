...
#include "tqt_igemmcf_mx1bias_clamp_f16_f16p_f16_interface.h"

namespace
{
/// Micro-kernel interface
const tqt_igemmcf_mx1bias_clamp_f16_f16p_f16_ukernel ukernel{
    tqt_get_m_step_igemmcf_mx1bias_clamp_f16_f16p_f16_3vlx8_rvv,
    tqt_get_n_step_igemmcf_mx1bias_clamp_f16_f16p_f16_3vlx8_rvv,
    tqt_get_a_packed_offset_igemmcf_mx1bias_clamp_f16_f16p_f16_3vlx8_rvv,
    tqt_get_c_offset_igemmcf_mx1bias_clamp_f16_f16p_f16_3vlx8_rvv,
    tqt_get_bias_offset_igemmcf_mx1bias_clamp_f16_f16p_f16_3vlx8_rvv,
    tqt_get_d_offset_igemmcf_mx1bias_clamp_f16_f16p_f16_3vlx8_rvv,
    tqt_get_d_size_igemmcf_mx1bias_clamp_f16_f16p_f16_3vlx8_rvv,
        ...
        for (size_t od = 0; od < OD; ++od) {
            for (size_t oh = 0; oh < OH; ++oh) {
                for (size_t ow = 0; ow < OW; ++ow) {
                            ...
                            for (size_t kh = 0; kh < KH; ++kh) {
                                for (size_t kw = 0; kw < KW; ++kw) {
                                    ...
                                    if (id >= 0 && id < (int)ID && ih >= 0 && ih < (int)IH &&
                                        iw >= 0 && iw < (int)IW) {
                                        acc += (float)activation[ic * ID * IH * IW +
                                                                 (size_t)id * IH * IW +
                                                                 (size_t)ih * IW + (size_t)iw] *
                                               (float)weight[oc * IC_pg * KD * KH * KW +
                                                             ic * KD * KH * KW + kd * KH * KW +
                                                             kh * KW + kw];
                                    }
                                }
                            }
                    ...
                    output[oc * OD * OH * OW + od * OH * OW + oh * OW + ow] = (float16_t)acc;
                }
            }
        }
    }
...
int main()
{
    ...
    reference_conv_cf(activation, weight, bias, ref_output, &params, clamp_min, clamp_max);

    // Pack weight [M, K] -> packed format
    size_t w_packed_size = ukernel.get_a_packed_size(M, K);
    void *w_packed = malloc(w_packed_size);
    ukernel.run_a_pack(M, K, K, K, 0, weight, w_packed);

    // Run igemmcf micro-kernel with packed weight (no im2col needed)
    ...
    printf("TEST[igemmcf_mx1bias_clamp_f16_f16p_f16]\n");
    printf("- ukernel: igemmcf_mx1bias_clamp_f16_f16p_f16_3vlx8_rvv\n");
