...
#include "tqt_igemmcl_1xnbias_clamp_f32_f32_f32p_8x3vl_rvv.h"
                                                                  ...
                                                                  void *weight_packed)
{
    ...
    tqt_gemm_pack_nxk_1x3vl_e32_rvv(n, k, ldw, ldw_packed, k_idx, weight, weight_packed);
}
        ...
        for (size_t kh = 0; kh < KH; kh++) {
            ...
            for (size_t kw = 0; kw < KW; kw++) {
                ...
                for (size_t ic = 0; ic < IC; ic++) {
                    ...
                    tqt_load_b_kernel_gemm_f32_f32_8x3vl_rvv<NR>(vl, w_ptr, vlmax);
...
using igemmcl_packed_kernel_fn_t = void (*)(
    size_t IC, size_t IC_stride, size_t kd_start, size_t kd_end, size_t KH, size_t KW, float *D,
    const float *W, const float *input_group, const float *C, const float *bias, size_t ldd,
    size_t ldc, float clamp_min, float clamp_max, size_t vl, const int *od_arr, const int *oh_arr,
    const int *ow_arr, size_t ID, size_t IH, size_t IW, size_t SD, size_t SH, size_t SW, size_t DD,
    size_t DH, size_t DW, size_t PD, size_t PH, size_t PW, bool is_first_k, bool is_last_k);
...
static void igemmcl_packed_dispatch(size_t M, size_t N, size_t IC, size_t IC_stride,
                                    size_t kd_start, size_t kd_end, size_t KH, size_t KW, float *D,
    ...
    size_t ldd, const void *bias, float clamp_min, float clamp_max, const tqt_conv_params *params)
{
    ...
    const float *input_group = (const float *)input + group_idx * IC_per_group;
    ...
    for (size_t m_idx_local = 0; m_idx_local < m; m_idx_local += m_step) {
        ...
        for (size_t n_idx = 0; n_idx < n; n_idx += n_step) {
            ...
            if (actual_n == n_step || k_idx_b == 0) {
                w_ptr = (const float *)B_packed + n_idx * ldb;
            } else {
                w_ptr = (const float *)B_packed + n_idx * ldb - k_idx_b * (n_step - actual_n);
            }
