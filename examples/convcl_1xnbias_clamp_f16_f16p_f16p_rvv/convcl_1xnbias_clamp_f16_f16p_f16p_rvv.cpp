...
#include "tqt_convcl_1xnbias_clamp_f16_f16p_f16p_interface.h"

namespace
{
/// Micro-kernel interface
const tqt_convcl_1xnbias_clamp_f16_f16p_f16p_ukernel ukernel{
    tqt_get_m_step_convcl_1xnbias_clamp_f16_f16p_f16p_8x3vl_rvv,
    tqt_get_n_step_convcl_1xnbias_clamp_f16_f16p_f16p_8x3vl_rvv,
    tqt_get_a_im2colpack_offset_convcl_1xnbias_clamp_f16_f16p_f16p_8x3vl_rvv,
    tqt_get_b_packed_offset_convcl_1xnbias_clamp_f16_f16p_f16p_8x3vl_rvv,
    tqt_get_c_offset_convcl_1xnbias_clamp_f16_f16p_f16p_8x3vl_rvv,
    tqt_get_bias_offset_convcl_1xnbias_clamp_f16_f16p_f16p_8x3vl_rvv,
    tqt_get_d_offset_convcl_1xnbias_clamp_f16_f16p_f16p_8x3vl_rvv,
    tqt_get_d_size_convcl_1xnbias_clamp_f16_f16p_f16p_8x3vl_rvv,
...
// Naive reference conv (CL layout)
        ...
        for (size_t oh = 0; oh < OH; ++oh) {
            for (size_t ow = 0; ow < OW; ++ow) {
                for (size_t oc = 0; oc < OC_pg; ++oc) {
                    ...
                    acc = fmaxf(acc, static_cast<float>(cmin));
                    acc = fminf(acc, static_cast<float>(cmax));
                    output[(od * OH * OW + oh * OW + ow) * OC_pg + oc] =
                        static_cast<float16_t>(acc);
                }
            }
        }
    }
...
int main()
{
    ...
    size_t IC = params.input_channel;
    ...
    size_t M = OD * OH * OW;
    size_t N = OC_pg;
    size_t K = IC * params.kernel_shape[0] * params.kernel_shape[1] * params.kernel_shape[2];

    // Allocate the memory
    // CL layout: [ID, IH, IW, IC]
    float16_t *activation = (float16_t *)malloc(ID * IH * IW * IC * sizeof(float16_t));
    ...
    // Initialize data
    fill_random(activation, ID * IH * IW * IC, 42);
    fill_random(weight, OC_pg * K, 123);
    fill_random(bias, OC_pg, 456);

    // Run reference implementation
    float16_t cmin = (float16_t)(-60000.0f);
    float16_t cmax = (float16_t)(60000.0f);
    reference_conv_cl(activation, weight, bias, ref_output, &params, cmin, cmax);

    // Run micro-kernel implementation using ukernel interface
    // Fused im2col+pack A (activation) -> A_packed
    size_t a_packed_size = ukernel.get_a_im2colpack_size(M, K);
    void *A_packed = malloc(a_packed_size);
    ukernel.run_a_im2colpack(0, activation, K, 0, A_packed, &params);

    // Pack B (weight) [N, K] transposed
    size_t b_packed_size = ukernel.get_b_packed_size(N, K);
    void *B_packed = malloc(b_packed_size);
    ukernel.run_bt_pack(N, K, K, K, 0, weight, B_packed);

    ukernel.run_conv(M, N, K, A_packed, K, 0, B_packed, K, 0, NULL, N, act_output, N, bias, cmin,
                     cmax);
    ...
    printf("TEST[convcl_1xnbias_clamp_f16_f16p_f16p]\n");
    printf("- ukernel: convcl_1xnbias_clamp_f16_f16p_f16p_8x3vl_rvv\n");
