...
#include "tqt_convcf_mx1bias_reqi32toi8_clamp_qai8_qsi8dxp_qai8p_4x1vl_rvv.h"
...
{
    return tqt_get_m_step_gemm_mx1bias_reqi32toi8_clamp_qai8_qsi8dxp_qai8p_4x1vl_rvv();
}

size_t tqt_get_n_step_convcf_mx1bias_reqi32toi8_clamp_qai8_qsi8dxp_qai8p_4x1vl_rvv(void)
{
    return tqt_get_n_step_gemm_mx1bias_reqi32toi8_clamp_qai8_qsi8dxp_qai8p_4x1vl_rvv();
}
...
{
    return tqt_get_a_packed_offset_gemm_mx1bias_reqi32toi8_clamp_qai8_qsi8dxp_qai8p_4x1vl_rvv(
        m_idx, k_idx, lda, actual_m);
}
...
size_t tqt_get_c_offset_convcf_mx1bias_reqi32toi8_clamp_qai8_qsi8dxp_qai8p_4x1vl_rvv(size_t m_idx,
                                                                                     ...
                                                                                     size_t ldc)
{
    return tqt_get_c_offset_gemm_mx1bias_reqi32toi8_clamp_qai8_qsi8dxp_qai8p_4x1vl_rvv(m_idx, n_idx,
                                                                                       ldc);
}
...
size_t tqt_get_bias_offset_convcf_mx1bias_reqi32toi8_clamp_qai8_qsi8dxp_qai8p_4x1vl_rvv(
    size_t m_idx)
{
    return tqt_get_bias_offset_gemm_mx1bias_reqi32toi8_clamp_qai8_qsi8dxp_qai8p_4x1vl_rvv(m_idx);
}
...
size_t tqt_get_d_offset_convcf_mx1bias_reqi32toi8_clamp_qai8_qsi8dxp_qai8p_4x1vl_rvv(size_t m_idx,
                                                                                     ...
                                                                                     size_t ldd)
{
    return tqt_get_d_offset_gemm_mx1bias_reqi32toi8_clamp_qai8_qsi8dxp_qai8p_4x1vl_rvv(m_idx, n_idx,
                                                                                       ldd);
}
...
size_t tqt_get_d_size_convcf_mx1bias_reqi32toi8_clamp_qai8_qsi8dxp_qai8p_4x1vl_rvv(size_t m,
                                                                                   size_t n)
{
    return tqt_get_d_size_gemm_mx1bias_reqi32toi8_clamp_qai8_qsi8dxp_qai8p_4x1vl_rvv(m, n);
}

size_t tqt_get_a_packed_size_convcf_mx1bias_reqi32toi8_clamp_qai8_qsi8dxp_qai8p_4x1vl_rvv(size_t m,
                                                                                          size_t k)
{
    return tqt_get_a_packed_size_gemm_mx1bias_reqi32toi8_clamp_qai8_qsi8dxp_qai8p_4x1vl_rvv(m, k);
}

size_t tqt_get_b_im2colpack_size_convcf_mx1bias_reqi32toi8_clamp_qai8_qsi8dxp_qai8p_4x1vl_rvv(
    size_t n, size_t k)
{
    return tqt_get_b_packed_size_gemm_mx1bias_reqi32toi8_clamp_qai8_qsi8dxp_qai8p_4x1vl_rvv(n, k);
}
...
{
    return tqt_get_b_packed_offset_gemm_mx1bias_reqi32toi8_clamp_qai8_qsi8dxp_qai8p_4x1vl_rvv(
        n_idx, k_idx, ldb, actual_n);
}
...
{
    tqt_run_a_pack_gemm_mx1bias_reqi32toi8_clamp_qai8_qsi8dxp_qai8p_4x1vl_rvv(m, k, lda, lda_packed,
                                                                              k_idx, A, A_packed);
}
...
static void b_im2colpack_cf_fill(int8_t *dst, size_t sub_n, size_t col_start,
                                 const int8_t *input_group, size_t IC_per_group,
        ...
        for (size_t kd = 0; kd < KD; ++kd) {
            for (size_t kh = 0; kh < KH; ++kh) {
                for (size_t kw = 0; kw < KW; ++kw) {
                    int8_t *row_dst = dst + k_row * sub_n;
                    ...
                    while (remaining > 0) {
                        ...
                        if (id_in < 0 || id_in >= (int)ID || ih_in < 0 || ih_in >= (int)IH) {
                            memset(row_dst + wp, 0, cnt * sizeof(int8_t));
                        } else {
                            ...
                            if (SW == 1 && DW == 1) {
                                ...
                                size_t vc = cnt - pl - pr;
                                if (pl)
                                    memset(row_dst + wp, 0, pl * sizeof(int8_t));
                                if (vc) {
                                    const int8_t *s = input_group + ic * ID * IH * IW +
                                                      (size_t)id_in * IH * IW + (size_t)ih_in * IW +
                                                      (size_t)(iw_s + (int)pl);
                                    int8_t *d = row_dst + wp + pl;
                                    size_t r = vc;
                                    while (r > 0) {
                                        size_t vl = __riscv_vsetvl_e8m1(r);
                                        __riscv_vse8_v_i8m1(d, __riscv_vle8_v_i8m1(s, vl), vl);
                                        s += vl;
                                        d += vl;
                                        r -= vl;
                                    }
                                ...
                                if (pr)
                                    memset(row_dst + wp + cnt - pr, 0, pr * sizeof(int8_t));
                            } else {
    ...
    const tqt_conv_params *params)
{
    ...
    const int8_t *input = (const int8_t *)activation;
    const int8_t *input_group = input + group_idx * IC_per_group * params->input_shape[0] *
                                            params->input_shape[1] * params->input_shape[2];
    int8_t *packed = (int8_t *)B_packed;

    const size_t vlmax = csrr_vlenb();  // int8: vlenb / sizeof(int8_t) = vlenb
    const size_t n_step_local = 1 * vlmax;

    for (size_t n_idx = 0; n_idx < N; n_idx += n_step_local) {
        const size_t actual_n = TQT_MIN(N - n_idx, n_step_local);
        int8_t *panel_ptr;
        if (n_idx == 0 || k_idx == 0 || actual_n == n_step_local)
            panel_ptr = packed + n_idx * ldb_packed;
        else
            panel_ptr = packed + n_idx * ldb_packed - k_idx * (n_step_local - actual_n);

        size_t size_n = (actual_n >= vlmax) ? vlmax : 0;
    ...
    const struct tqt_qai8_qsi8dx_qai8_params *params)
{
    tqt_run_gemm_mx1bias_reqi32toi8_clamp_qai8_qsi8dxp_qai8p_4x1vl_rvv(
        m, n, k, A_packed, lda, k_idx_a, B_packed, ldb, k_idx_b, C, ldc, D, ldd, bias, clamp_min,
        clamp_max, params);
}
