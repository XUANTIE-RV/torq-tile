...
#include "tqt_igemmcl_1xnbias_clamp_f32_f32_f32t_interface.h"

namespace
{
/// Micro-kernel interface
const tqt_igemmcl_1xnbias_clamp_f32_f32_f32t_ukernel ukernel{
    tqt_get_m_step_igemmcl_1xnbias_clamp_f32_f32_f32t_8x3vl_rvv,
    tqt_get_n_step_igemmcl_1xnbias_clamp_f32_f32_f32t_8x3vl_rvv,
    tqt_get_b_offset_igemmcl_1xnbias_clamp_f32_f32_f32t_8x3vl_rvv,
    tqt_get_c_offset_igemmcl_1xnbias_clamp_f32_f32_f32t_8x3vl_rvv,
    tqt_get_bias_offset_igemmcl_1xnbias_clamp_f32_f32_f32t_8x3vl_rvv,
    tqt_get_d_offset_igemmcl_1xnbias_clamp_f32_f32_f32t_8x3vl_rvv,
    tqt_get_d_size_igemmcl_1xnbias_clamp_f32_f32_f32t_8x3vl_rvv,
                            ...
                            for (size_t kw = 0; kw < KW; ++kw) {
                                ...
                                if (id >= 0 && id < (int)ID && ih >= 0 && ih < (int)IH && iw >= 0 &&
                                    ...
                                    for (size_t ic = 0; ic < IC_pg; ++ic) {
                                        ...
                                        float in_val = activation[((size_t)id * IH * IW +
                                                                   (size_t)ih * IW + (size_t)iw) *
                                                                      IC_pg +
                                                                  ic];
                                        // Weight: [OC, KD*KH*KW*IC] row-major
                                        float w_val = weight[oc * KD * KH * KW * IC_pg +
                                                             kd * KH * KW * IC_pg +
                                                             kh * KW * IC_pg + kw * IC_pg + ic];
                                        acc += in_val * w_val;
                                    }
                                }
                            }
...
int main()
{
    ...
    float *activation = (float *)malloc(ID * IH * IW * IC_pg * sizeof(float));
    ...
    // Run igemmcl micro-kernel (no im2col needed - input fetched on-the-fly)
    // M = spatial (OD*OH*OW), N = OC, K = KD*KH*KW*IC
    // ldb = K (weight row stride), ldd = N (output row stride = OC)
    ukernel.run_igemm(M, N, K, activation, 0, 0, 0, weight, K, 0, NULL, N, act_output, N, bias,
                      -FLT_MAX, FLT_MAX, &params);

    // Calculate cosine similarity
    const float cosine_sim = calculate_cosine_similarity(M * N, ref_output, act_output);
    const bool passed = cosine_sim > 0.9999f;

    printf("TEST[igemmcl_1xnbias_clamp_f32_f32_f32t]\n");
    printf("- ukernel: igemmcl_1xnbias_clamp_f32_f32_f32t_8x3vl_rvv\n");
