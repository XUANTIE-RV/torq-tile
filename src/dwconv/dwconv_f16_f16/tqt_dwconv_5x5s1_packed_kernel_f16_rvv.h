...
static inline void tqt_dwconv_5x5s1_load_input_8x1(vfloat16m1_t *r0, vfloat16m1_t *r1,
                                                   vfloat16m1_t *r2, vfloat16m1_t *r3,
                                                   vfloat16m1_t *r4, vfloat16m1_t *r5,
                                                   vfloat16m1_t *r6, vfloat16m1_t *r7,
                                                   const float16_t *col_base_in_block,
                                                   long ih_first, long IH_l, size_t IW, size_t vl)
{
...
#define TQT_LOAD_OR_ZERO(VAR, R)                                                           \
    do {                                                                                   \
        long ih = ih_first + (R);                                                          \
        VAR = (ih >= 0 && ih < IH_l)                                                       \
                  ? __riscv_vle16_v_f16m1(col_base_in_block + (size_t)ih * row_stride, vl) \
...
static inline void tqt_dwconv_5x5s1_zero_input_8x1(vfloat16m1_t *r0, vfloat16m1_t *r1,
                                                   vfloat16m1_t *r2, vfloat16m1_t *r3,
                                                   vfloat16m1_t *r4, vfloat16m1_t *r5,
                                                   vfloat16m1_t *r6, vfloat16m1_t *r7, size_t vl)
{
    *r0 = __riscv_vfmv_v_f_f16m1((float16_t)0.0f, vl);
    *r1 = __riscv_vfmv_v_f_f16m1((float16_t)0.0f, vl);
    *r2 = __riscv_vfmv_v_f_f16m1((float16_t)0.0f, vl);
    *r3 = __riscv_vfmv_v_f_f16m1((float16_t)0.0f, vl);
    *r4 = __riscv_vfmv_v_f_f16m1((float16_t)0.0f, vl);
    *r5 = __riscv_vfmv_v_f_f16m1((float16_t)0.0f, vl);
    *r6 = __riscv_vfmv_v_f_f16m1((float16_t)0.0f, vl);
    *r7 = __riscv_vfmv_v_f_f16m1((float16_t)0.0f, vl);
}

static inline void tqt_dwconv_5x5s1_packed_kernel_f16_rvv(
    ...
    for (size_t oc1 = 0; oc1 < OC1; ++oc1) {
        ...
        for (; oh + 4 <= oh_end; oh += 4) {
            ...
            for (size_t ow = 0; ow < OW; ++ow) {
                vfloat16m1_t acc0 = v_bias, acc1 = v_bias, acc2 = v_bias, acc3 = v_bias;

                // KW loop: process one input/weight column at a time
                for (size_t kw = 0; kw < KW; ++kw) {
                    const long iw_in = (long)ow + (long)kw - (long)PW;

                    vfloat16m1_t i0, i1, i2, i3, i4, i5, i6, i7;
                    if (iw_in >= 0 && iw_in < (long)IW) {
                        const float16_t *col_base = in_block + (size_t)iw_in * vl;
                    ...
                    vfloat16m1_t w0 = __riscv_vle16_v_f16m1(wt_block + (0 * KW + kw) * vl, vl);
                    vfloat16m1_t w1 = __riscv_vle16_v_f16m1(wt_block + (1 * KW + kw) * vl, vl);
                    vfloat16m1_t w2 = __riscv_vle16_v_f16m1(wt_block + (2 * KW + kw) * vl, vl);
                    vfloat16m1_t w3 = __riscv_vle16_v_f16m1(wt_block + (3 * KW + kw) * vl, vl);
                    vfloat16m1_t w4 = __riscv_vle16_v_f16m1(wt_block + (4 * KW + kw) * vl, vl);

                    // acc[i] += sum_{kh=0..4} input_row[i+kh] * w[kh]
                    acc0 = __riscv_vfmacc_vv_f16m1(acc0, i0, w0, vl);
                    acc0 = __riscv_vfmacc_vv_f16m1(acc0, i1, w1, vl);
                    acc0 = __riscv_vfmacc_vv_f16m1(acc0, i2, w2, vl);
                    acc0 = __riscv_vfmacc_vv_f16m1(acc0, i3, w3, vl);
                    acc0 = __riscv_vfmacc_vv_f16m1(acc0, i4, w4, vl);

                    acc1 = __riscv_vfmacc_vv_f16m1(acc1, i1, w0, vl);
                    acc1 = __riscv_vfmacc_vv_f16m1(acc1, i2, w1, vl);
                    acc1 = __riscv_vfmacc_vv_f16m1(acc1, i3, w2, vl);
                    acc1 = __riscv_vfmacc_vv_f16m1(acc1, i4, w3, vl);
                    acc1 = __riscv_vfmacc_vv_f16m1(acc1, i5, w4, vl);

                    acc2 = __riscv_vfmacc_vv_f16m1(acc2, i2, w0, vl);
                    acc2 = __riscv_vfmacc_vv_f16m1(acc2, i3, w1, vl);
                    acc2 = __riscv_vfmacc_vv_f16m1(acc2, i4, w2, vl);
                    acc2 = __riscv_vfmacc_vv_f16m1(acc2, i5, w3, vl);
                    acc2 = __riscv_vfmacc_vv_f16m1(acc2, i6, w4, vl);

                    acc3 = __riscv_vfmacc_vv_f16m1(acc3, i3, w0, vl);
                    acc3 = __riscv_vfmacc_vv_f16m1(acc3, i4, w1, vl);
                    acc3 = __riscv_vfmacc_vv_f16m1(acc3, i5, w2, vl);
                    acc3 = __riscv_vfmacc_vv_f16m1(acc3, i6, w3, vl);
                    acc3 = __riscv_vfmacc_vv_f16m1(acc3, i7, w4, vl);
                }
            ...
            for (size_t ow = 0; ow < OW; ++ow) {
                ...
                for (size_t kh = 0; kh < KH; ++kh) {
                    ...
                    for (size_t kw = 0; kw < KW; ++kw) {
                        ...
                        if (iw_in < 0 || iw_in >= (long)IW)
                            continue;
                        const float16_t *in_pos =
                            in_block + ((size_t)ih_in * IW + (size_t)iw_in) * vl;
                        const float16_t *wt_pos = wt_block + (kh * KW + kw) * vl;
