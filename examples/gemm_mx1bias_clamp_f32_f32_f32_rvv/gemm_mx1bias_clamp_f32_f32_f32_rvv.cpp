...
#include "tqt_gemm_mx1bias_clamp_f32_f32_f32_interface.h"

namespace
{
/// Micro-kernel interface
const tqt_gemm_mx1bias_clamp_f32_f32_f32_ukernel ukernel{
    ...
    for (size_t i = 0; i < m; ++i) {
        for (size_t j = 0; j < n; ++j) {
            ...
            for (size_t p = 0; p < k; ++p) {
                ...
                float b_val = B[p * ldb + j];  // B is k x n
                acc += a_val * b_val;
            }
...
int main()
{
    ...
    float *B = (float *)malloc(K * N * sizeof(float));  // B is k x n
    float *C = (float *)malloc(M * N * sizeof(float));
    float *bias = (float *)malloc(M * sizeof(float));  // Mx1 bias
    ...
    printf("TEST[gemm_mx1bias_clamp_f32_f32_f32]\n");
    printf("- ukernel: gemm_mx1bias_clamp_f32_f32_f32_8x3vl_rvv\n");
