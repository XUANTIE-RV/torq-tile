...
// Include micro-kernel variants (B-only packed, B input is transposed [N, K])
...
/// Micro-kernel interface (B-only packed: A original, B packed from transposed [N,K])
...
// Note: B is [N,K] (transposed storage)
...
// weight_i8 is [num_channels, channel_size] (transposed storage, each row is a channel).
                     ...
                     int32_t zero_point, size_t num_channels, size_t channel_size)
{
    for (size_t ch = 0; ch < num_channels; ++ch) {
        ...
        for (size_t ki = 0; ki < channel_size; ++ki) {
            weight_sum += (int32_t)weight_i8[ch * channel_size + ki];
        }
...
int main()
{
    ...
    float *B_fp32 = (float *)malloc(N * K * sizeof(float));  // B is [N, K] (transposed)
    ...
    fill_matrix_random(N, K, B_fp32, -1.0f, 1.0f);  // B is [N, K]
    ...
    // B: per-channel symmetric (each row of B[N,K] is a channel)
    ...
    int8_t *BT_i8 = (int8_t *)malloc(N * K);  // B in [N, K] layout
    ...
    // Quantize B per-channel: each row of B[N,K] uses scale_b[ch]
    for (size_t ch = 0; ch < N; ++ch) {
    ...
    // B-only packed: pack B from [N, K] layout via run_bt_pack
    ...
    printf("TEST[gemm_1xnbias_reqi32toi8_clamp_qai8_qai8_qsi8cxp_pack_bt]\n");
