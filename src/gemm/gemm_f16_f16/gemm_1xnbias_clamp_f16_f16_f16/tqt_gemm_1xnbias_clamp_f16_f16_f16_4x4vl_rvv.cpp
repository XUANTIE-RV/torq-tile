...
#include "tqt_gemm_1xnbias_clamp_f16_f16_f16_4x4vl_rvv.h"
                            ...
                            size_t vl)
{
    ...
    tqt_load_b_kernel_gemm_f16_f16_4x4vl_rvv(vl, b_ptr);
    a_ptr += 1;
    b_ptr += ldb;

    // Steady-state K-loop with software pipelining
    for (size_t k_idx = 1; k_idx < K; k_idx++) {
        tqt_fused_load_vfmacc_axb_kernel_gemm_f16_f16_4x4vl_rvv<MR>(vl, a_ptr, a_stride_row, b_ptr);
        a_ptr += 1;
        b_ptr += ldb;
    }
