...
#include "tqt_gemm_mx1bias_reqi32toi8_clamp_qai8_qsi8dxp_qai8p_4x1vl_rvv.h"
...
size_t tqt_get_m_step_gemm_mx1bias_reqi32toi8_clamp_qai8_qsi8dxp_qai8p_4x1vl_rvv(void)
{
    return m_step;
}

size_t tqt_get_n_step_gemm_mx1bias_reqi32toi8_clamp_qai8_qsi8dxp_qai8p_4x1vl_rvv(void)
{
    return n_step;
}

size_t tqt_get_a_packed_offset_gemm_mx1bias_reqi32toi8_clamp_qai8_qsi8dxp_qai8p_4x1vl_rvv(
...
size_t tqt_get_b_packed_offset_gemm_mx1bias_reqi32toi8_clamp_qai8_qsi8dxp_qai8p_4x1vl_rvv(
...
size_t tqt_get_c_offset_gemm_mx1bias_reqi32toi8_clamp_qai8_qsi8dxp_qai8p_4x1vl_rvv(size_t m_idx,
                                                                                   size_t n_idx,
                                                                                   size_t ldc)
{
    TQT_ASSUME(m_idx % m_step == 0);
...
size_t tqt_get_bias_offset_gemm_mx1bias_reqi32toi8_clamp_qai8_qsi8dxp_qai8p_4x1vl_rvv(size_t m_idx)
{
    TQT_ASSUME(m_idx % m_step == 0);
    return m_idx * sizeof(int32_t);
}

size_t tqt_get_d_offset_gemm_mx1bias_reqi32toi8_clamp_qai8_qsi8dxp_qai8p_4x1vl_rvv(size_t m_idx,
                                                                                   size_t n_idx,
                                                                                   size_t ldd)
{
    TQT_ASSUME(m_idx % m_step == 0);
...
size_t tqt_get_d_size_gemm_mx1bias_reqi32toi8_clamp_qai8_qsi8dxp_qai8p_4x1vl_rvv(size_t m, size_t n)
{
    return m * n * sizeof(int8_t);
}

size_t tqt_get_a_packed_size_gemm_mx1bias_reqi32toi8_clamp_qai8_qsi8dxp_qai8p_4x1vl_rvv(size_t m,
                                                                                        size_t k)
{
    return m * k * sizeof(int8_t);
}

size_t tqt_get_b_packed_size_gemm_mx1bias_reqi32toi8_clamp_qai8_qsi8dxp_qai8p_4x1vl_rvv(size_t n,
                                                                                        size_t k)
{
...
void tqt_run_a_pack_gemm_mx1bias_reqi32toi8_clamp_qai8_qsi8dxp_qai8p_4x1vl_rvv(
...
void tqt_run_b_pack_gemm_mx1bias_reqi32toi8_clamp_qai8_qsi8dxp_qai8p_4x1vl_rvv(
...
void tqt_run_bt_pack_gemm_mx1bias_reqi32toi8_clamp_qai8_qsi8dxp_qai8p_4x1vl_rvv(
    size_t n, size_t k, size_t ldb, size_t ldb_packed, size_t k_idx, const void *B, void *B_packed)
{
...
// Single GEMM kernel iteration for MRxvl tile with packed inputs and requantization
template <size_t MR>
                        ...
                        const int32_t *shift, int32_t zp)
{
    ...
    if (BIAS) {
        tqt_add_mx1bias_kernel_gemm_i8_i8_4x1vl_rvv<MR>(vl, BIAS);
    }

    tqt_requantize_mx1_kernel_gemm_i8_i8_4x1vl_rvv<MR>(vl, mult, shift, zp);
...
void tqt_run_gemm_mx1bias_reqi32toi8_clamp_qai8_qsi8dxp_qai8p_4x1vl_rvv(
    ...
    const struct tqt_qai8_qsi8dx_qai8_params *params)
{
    ...
    // Pre-compute requantization parameters for all rows
    // real_scale = scale_a[i] * scale_b / scale_d
    int32_t mult[m];
    int32_t shift[m];
    double real_scale =
        (double)params->scale_a[0] * (double)params->scale_b / (double)params->scale_d;
    tqt_quantize_multiplier(real_scale, &mult[0], &shift[0]);
    shift[0] = -shift[0] - 1;
    for (size_t i = 1; i < m; i++) {
        if (params->quant_channel_a == 1) {
            mult[i] = mult[0];
            shift[i] = shift[0];
        } else {
            real_scale =
                (double)params->scale_a[i] * (double)params->scale_b / (double)params->scale_d;
    ...
    for (size_t m_idx = 0; m_idx < m; m_idx += m_step) {
        for (size_t n_idx = 0; n_idx < n; n_idx += n_step) {
            ...
            const int32_t *bias_tile_ptr = bias_ptr ? bias_ptr + m_idx : NULL;
            int8_t *d_ptr = D_ptr + m_idx * ldd + n_idx;

            const int32_t *mult_ptr = mult + m_idx;
            const int32_t *shift_ptr = shift + m_idx;
