...
#include "tqt_gemm_1xnbias_clamp_f32_f32_f32_4x4vl_rvv.h"
...
static const size_t n_step = 4 * vlmax;  // LMUL=4: vlmax * 4 elements per iteration
...
// Template-based GEMM kernel dispatcher for 4x4vl (LMUL=4).
                            ...
                            float clamp_min, float clamp_max, size_t vl)
{
    ...
    tqt_load_b_kernel_gemm_f32_f32_4x4vl_rvv(vl, b_ptr);
    a_ptr += 1;
    b_ptr += ldb;

    // Steady-state K-loop with software pipelining
    for (size_t k_idx = 1; k_idx < K; k_idx++) {
        tqt_fused_load_vfmacc_axb_kernel_gemm_f32_f32_4x4vl_rvv<MR>(vl, a_ptr, a_stride_row, b_ptr);
        a_ptr += 1;
        b_ptr += ldb;
    }
                                                      ...
                                                      float clamp_max)
{
    ...
    for (size_t m_idx = 0; m_idx < m; m_idx += m_step) {
        for (size_t n_idx = 0; n_idx < n; n_idx += n_step) {
            ...
            // in [1, n_step] via RVV variable-length vector semantics. Tail-agnostic
            // mode (ta) ensures elements past vl are not written back.
