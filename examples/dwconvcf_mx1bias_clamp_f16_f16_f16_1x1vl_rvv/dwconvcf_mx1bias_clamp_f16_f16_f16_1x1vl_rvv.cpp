...
#include "tqt_dwconvcf_mx1bias_clamp_f16_f16_f16_interface.h"

namespace
{

const tqt_dwconvcf_mx1bias_clamp_f16_f16_f16_ukernel ukernel{
...
int main(void)
{
    ...
    float16_t *bias = (float16_t *)std::malloc(bs_count * sizeof(float16_t));
    ...
    const float16_t cmin = (float16_t)0.0f;  // ReLU
    const float16_t cmax = FLT_MAX;
    ...
    if (out_bytes != out_count * sizeof(float16_t)) {
        std::printf("FAIL: get_output_size mismatch (got %zu, expected %zu)\n", out_bytes,
                    out_count * sizeof(float16_t));
        return 1;
    }
    ...
    std::printf("TEST[dwconvcf_mx1bias_clamp_f16_f16_f16]\n");
    std::printf("- ukernel: dwconvcf_mx1bias_clamp_f16_f16_f16_1x1vl_rvv\n");
