...
#include "tqt_dwconvcf3x3s2_mx1bias_clamp_f32p_f32p_f32p_interface.h"

namespace
{
const tqt_dwconvcf3x3s2_mx1bias_clamp_f32p_f32p_f32p_ukernel ukernel{
            ...
            for (size_t ow = 0; ow < OW; ++ow) {
                ...
                for (size_t kh = 0; kh < KH; ++kh)
                    for (size_t kw = 0; kw < KW; ++kw) {
                        int ih = (int)(oh * 2 + kh) - (int)p->pad[1];
                        int iw = (int)(ow * 2 + kw) - (int)p->pad[2];
...
int main(void)
{
    ...
    reference_dwconv_cf(input, weight, bias, out_ref, &p, 0.0f, FLT_MAX);
    ...
    std::free(p_wt);
    std::free(p_out);
    std::printf("TEST[dwconvcf3x3s2_mx1bias_clamp_f32p_f32p_f32p]\n");
    std::printf("- ukernel: dwconvcf3x3s2_mx1bias_clamp_f32p_f32p_f32p_2x1x1vl_rvv\n");
