...
/// Micro-kernel: GEMM with clamp for INT8 -> INT32
///
/// This is a rvv (4 x vl tile) implementation where vl = csrr_vlenb() / sizeof(int8).
/// Computes: D = clamp(C + A*B, min, max)
/// where A and B are int8, and D is int32.
...
/// Runs the GEMM microkernel with clamp operation.
///
/// Computes: D = clamp(C + A*B, clamp_min, clamp_max)
///
