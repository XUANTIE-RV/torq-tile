...
#define TQT_VZERO_FP32(reg_num) asm volatile("vmv.v.i v" #reg_num ", 0\n\t" : : : "v" #reg_num);

/// Initialize accumulator registers to zero
template <size_t MR, size_t NR>
static inline void tqt_init_zero_kernel_gemm_f32_f32_8x3vl_rvv(size_t vl)
{
...
template <size_t MR, size_t NR>
static inline void tqt_init_c_kernel_gemm_f32_f32_8x3vl_rvv(size_t vl, const float *C,
                                                            size_t stride_row, size_t stride_col)
{
    ...
    if constexpr (MR >= 1) {
        ...
        if constexpr (NR >= 1)
            asm volatile("vle32.v v8, (%0)" : : "r"(c_row_0 + 0 * stride_col) : "v8", "memory");
        if constexpr (NR >= 2)
            asm volatile("vle32.v v9, (%0)" : : "r"(c_row_0 + 1 * stride_col) : "v9", "memory");
        if constexpr (NR >= 3)
            asm volatile("vle32.v v10, (%0)" : : "r"(c_row_0 + 2 * stride_col) : "v10", "memory");
    }
    // Row 1
    if constexpr (MR >= 2) {
        const float *c_row_1 = C + 1 * stride_row;
        if constexpr (NR >= 1)
            asm volatile("vle32.v v11, (%0)" : : "r"(c_row_1 + 0 * stride_col) : "v11", "memory");
        if constexpr (NR >= 2)
            asm volatile("vle32.v v12, (%0)" : : "r"(c_row_1 + 1 * stride_col) : "v12", "memory");
        if constexpr (NR >= 3)
            asm volatile("vle32.v v13, (%0)" : : "r"(c_row_1 + 2 * stride_col) : "v13", "memory");
    }
    // Row 2
    if constexpr (MR >= 3) {
        const float *c_row_2 = C + 2 * stride_row;
        if constexpr (NR >= 1)
            asm volatile("vle32.v v14, (%0)" : : "r"(c_row_2 + 0 * stride_col) : "v14", "memory");
        if constexpr (NR >= 2)
            asm volatile("vle32.v v15, (%0)" : : "r"(c_row_2 + 1 * stride_col) : "v15", "memory");
        if constexpr (NR >= 3)
            asm volatile("vle32.v v16, (%0)" : : "r"(c_row_2 + 2 * stride_col) : "v16", "memory");
    }
    // Row 3
    if constexpr (MR >= 4) {
        const float *c_row_3 = C + 3 * stride_row;
        if constexpr (NR >= 1)
            asm volatile("vle32.v v17, (%0)" : : "r"(c_row_3 + 0 * stride_col) : "v17", "memory");
        if constexpr (NR >= 2)
            asm volatile("vle32.v v18, (%0)" : : "r"(c_row_3 + 1 * stride_col) : "v18", "memory");
        if constexpr (NR >= 3)
            asm volatile("vle32.v v19, (%0)" : : "r"(c_row_3 + 2 * stride_col) : "v19", "memory");
    }
    // Row 4
    if constexpr (MR >= 5) {
        const float *c_row_4 = C + 4 * stride_row;
        if constexpr (NR >= 1)
            asm volatile("vle32.v v20, (%0)" : : "r"(c_row_4 + 0 * stride_col) : "v20", "memory");
        if constexpr (NR >= 2)
            asm volatile("vle32.v v21, (%0)" : : "r"(c_row_4 + 1 * stride_col) : "v21", "memory");
        if constexpr (NR >= 3)
            asm volatile("vle32.v v22, (%0)" : : "r"(c_row_4 + 2 * stride_col) : "v22", "memory");
    }
    // Row 5
    if constexpr (MR >= 6) {
        const float *c_row_5 = C + 5 * stride_row;
        if constexpr (NR >= 1)
            asm volatile("vle32.v v23, (%0)" : : "r"(c_row_5 + 0 * stride_col) : "v23", "memory");
        if constexpr (NR >= 2)
            asm volatile("vle32.v v24, (%0)" : : "r"(c_row_5 + 1 * stride_col) : "v24", "memory");
        if constexpr (NR >= 3)
            asm volatile("vle32.v v25, (%0)" : : "r"(c_row_5 + 2 * stride_col) : "v25", "memory");
    }
    // Row 6
    if constexpr (MR >= 7) {
        const float *c_row_6 = C + 6 * stride_row;
        if constexpr (NR >= 1)
            asm volatile("vle32.v v26, (%0)" : : "r"(c_row_6 + 0 * stride_col) : "v26", "memory");
        if constexpr (NR >= 2)
            asm volatile("vle32.v v27, (%0)" : : "r"(c_row_6 + 1 * stride_col) : "v27", "memory");
        if constexpr (NR >= 3)
            asm volatile("vle32.v v28, (%0)" : : "r"(c_row_6 + 2 * stride_col) : "v28", "memory");
    }
    // Row 7
    if constexpr (MR >= 8) {
        const float *c_row_7 = C + 7 * stride_row;
        if constexpr (NR >= 1)
            asm volatile("vle32.v v29, (%0)" : : "r"(c_row_7 + 0 * stride_col) : "v29", "memory");
        if constexpr (NR >= 2)
            asm volatile("vle32.v v30, (%0)" : : "r"(c_row_7 + 1 * stride_col) : "v30", "memory");
        if constexpr (NR >= 3)
            asm volatile("vle32.v v31, (%0)" : : "r"(c_row_7 + 2 * stride_col) : "v31", "memory");
    }
}

/// Load A matrix elements (scalars) into floating-point registers
template <size_t MR>
static inline void tqt_load_a_kernel_gemm_f32_f32_8x3vl_rvv(const float *A, size_t stride_row)
{
    static_assert(MR >= 1 && MR <= 8, "MR must be in range [1, 8]");
    ...
    if constexpr (MR >= 5)
        asm volatile("flw ft4, 0(%0)" : : "r"(A + 4 * stride_row) : "ft4", "memory");
    if constexpr (MR >= 6)
        asm volatile("flw ft5, 0(%0)" : : "r"(A + 5 * stride_row) : "ft5", "memory");
    if constexpr (MR >= 7)
        asm volatile("flw ft6, 0(%0)" : : "r"(A + 6 * stride_row) : "ft6", "memory");
    if constexpr (MR >= 8)
        asm volatile("flw ft7, 0(%0)" : : "r"(A + 7 * stride_row) : "ft7", "memory");
}

/// Load B matrix columns (vectors) into vector registers
template <size_t NR>
static inline void tqt_load_b_kernel_gemm_f32_f32_8x3vl_rvv(size_t vl, const float *B,
                                                            size_t stride_col)
{
    static_assert(NR >= 1 && NR <= 3, "NR must be in range [1, 3]");
    ...
    if constexpr (NR >= 1)
        asm volatile("vle32.v v0, (%0)" : : "r"(B + 0 * stride_col) : "v0", "memory");
    if constexpr (NR >= 2)
        asm volatile("vle32.v v1, (%0)" : : "r"(B + 1 * stride_col) : "v1", "memory");
    if constexpr (NR >= 3)
        asm volatile("vle32.v v2, (%0)" : : "r"(B + 2 * stride_col) : "v2", "memory");
}

/// Load transposed B matrix columns (vectors) into vector registers
template <size_t NR>
static inline void tqt_load_bt_kernel_gemm_f32_f32_8x3vl_rvv(size_t vl, const float *B,
                                                             size_t stride_row)
{
    static_assert(NR >= 1 && NR <= 3, "NR must be in range [1, 3]");
...
template <size_t MR, size_t NR>
static inline void tqt_vfmacc_kernel_gemm_f32_f32_8x3vl_rvv(size_t vl)
{
    static_assert(MR >= 1 && MR <= 8, "MR must be in range [1, 8]");
    static_assert(NR >= 1 && NR <= 3, "NR must be in range [1, 3]");
...
template <size_t MR, size_t NR>
static inline void tqt_add_1xnbias_kernel_gemm_f32_f32_8x3vl_rvv(size_t vl, const float *BIAS)
{
    static_assert(MR >= 1 && MR <= 8, "MR must be in range [1, 8]");
    static_assert(NR >= 1 && NR <= 3, "NR must be in range [1, 3]");
    ...
    if constexpr (NR >= 1)
        asm volatile("vle32.v v3, (%0)" : : "r"(BIAS + 0 * vl) : "v3", "memory");
    if constexpr (NR >= 2)
        asm volatile("vle32.v v4, (%0)" : : "r"(BIAS + 1 * vl) : "v4", "memory");
    if constexpr (NR >= 3)
        asm volatile("vle32.v v5, (%0)" : : "r"(BIAS + 2 * vl) : "v5", "memory");
...
template <size_t MR, size_t NR>
static inline void tqt_add_mx1bias_kernel_gemm_f32_f32_8x3vl_rvv(size_t vl, const float *BIAS)
{
    static_assert(MR >= 1 && MR <= 8, "MR must be in range [1, 8]");
    static_assert(NR >= 1 && NR <= 3, "NR must be in range [1, 3]");
    ...
    // Row 0
    if constexpr (MR >= 1) {
        asm volatile("flw ft0, 0(%0)" : : "r"(BIAS + 0) : "ft0", "memory");
        if constexpr (NR >= 1)
    ...
    // Row 1
    if constexpr (MR >= 2) {
        asm volatile("flw ft1, 0(%0)" : : "r"(BIAS + 1) : "ft1", "memory");
        if constexpr (NR >= 1)
    ...
    // Row 2
    if constexpr (MR >= 3) {
        asm volatile("flw ft2, 0(%0)" : : "r"(BIAS + 2) : "ft2", "memory");
        if constexpr (NR >= 1)
    ...
    // Row 3
    if constexpr (MR >= 4) {
        asm volatile("flw ft3, 0(%0)" : : "r"(BIAS + 3) : "ft3", "memory");
        if constexpr (NR >= 1)
    ...
    // Row 4
    if constexpr (MR >= 5) {
        asm volatile("flw ft4, 0(%0)" : : "r"(BIAS + 4) : "ft4", "memory");
        if constexpr (NR >= 1)
    ...
    // Row 5
    if constexpr (MR >= 6) {
        asm volatile("flw ft5, 0(%0)" : : "r"(BIAS + 5) : "ft5", "memory");
        if constexpr (NR >= 1)
    ...
    // Row 6
    if constexpr (MR >= 7) {
        asm volatile("flw ft6, 0(%0)" : : "r"(BIAS + 6) : "ft6", "memory");
        if constexpr (NR >= 1)
    ...
    // Row 7
    if constexpr (MR >= 8) {
        asm volatile("flw ft7, 0(%0)" : : "r"(BIAS + 7) : "ft7", "memory");
        if constexpr (NR >= 1)
...
template <size_t MR, size_t NR>
static inline void tqt_clamp_kernel_gemm_f32_f32_8x3vl_rvv(size_t vl, float min, float max)
{
    static_assert(MR >= 1 && MR <= 8, "MR must be in range [1, 8]");
    static_assert(NR >= 1 && NR <= 3, "NR must be in range [1, 3]");
...
template <size_t MR, size_t NR>
static inline void tqt_store_kernel_gemm_f32_f32_8x3vl_rvv(size_t vl, float *D, size_t stride_row,
                                                           size_t stride_col)
{
    static_assert(MR >= 1 && MR <= 8, "MR must be in range [1, 8]");
    static_assert(NR >= 1 && NR <= 3, "NR must be in range [1, 3]");
    ...
    if constexpr (MR >= 1) {
        float *d_row_0 = D + 0 * stride_row;
        if constexpr (NR >= 1)
            asm volatile("vse32.v v8, (%0)" : : "r"(d_row_0 + 0 * stride_col) : "memory");
        if constexpr (NR >= 2)
            asm volatile("vse32.v v9, (%0)" : : "r"(d_row_0 + 1 * stride_col) : "memory");
        if constexpr (NR >= 3)
            asm volatile("vse32.v v10, (%0)" : : "r"(d_row_0 + 2 * stride_col) : "memory");
    }
    if constexpr (MR >= 2) {
        float *d_row_1 = D + 1 * stride_row;
        if constexpr (NR >= 1)
            asm volatile("vse32.v v11, (%0)" : : "r"(d_row_1 + 0 * stride_col) : "memory");
        if constexpr (NR >= 2)
            asm volatile("vse32.v v12, (%0)" : : "r"(d_row_1 + 1 * stride_col) : "memory");
        if constexpr (NR >= 3)
            asm volatile("vse32.v v13, (%0)" : : "r"(d_row_1 + 2 * stride_col) : "memory");
    }
    if constexpr (MR >= 3) {
        float *d_row_2 = D + 2 * stride_row;
        if constexpr (NR >= 1)
            asm volatile("vse32.v v14, (%0)" : : "r"(d_row_2 + 0 * stride_col) : "memory");
        if constexpr (NR >= 2)
            asm volatile("vse32.v v15, (%0)" : : "r"(d_row_2 + 1 * stride_col) : "memory");
        if constexpr (NR >= 3)
            asm volatile("vse32.v v16, (%0)" : : "r"(d_row_2 + 2 * stride_col) : "memory");
    }
    if constexpr (MR >= 4) {
        float *d_row_3 = D + 3 * stride_row;
        if constexpr (NR >= 1)
            asm volatile("vse32.v v17, (%0)" : : "r"(d_row_3 + 0 * stride_col) : "memory");
        if constexpr (NR >= 2)
            asm volatile("vse32.v v18, (%0)" : : "r"(d_row_3 + 1 * stride_col) : "memory");
        if constexpr (NR >= 3)
            asm volatile("vse32.v v19, (%0)" : : "r"(d_row_3 + 2 * stride_col) : "memory");
    }
    if constexpr (MR >= 5) {
        float *d_row_4 = D + 4 * stride_row;
        if constexpr (NR >= 1)
            asm volatile("vse32.v v20, (%0)" : : "r"(d_row_4 + 0 * stride_col) : "memory");
        if constexpr (NR >= 2)
            asm volatile("vse32.v v21, (%0)" : : "r"(d_row_4 + 1 * stride_col) : "memory");
        if constexpr (NR >= 3)
            asm volatile("vse32.v v22, (%0)" : : "r"(d_row_4 + 2 * stride_col) : "memory");
    }
    if constexpr (MR >= 6) {
        float *d_row_5 = D + 5 * stride_row;
        if constexpr (NR >= 1)
            asm volatile("vse32.v v23, (%0)" : : "r"(d_row_5 + 0 * stride_col) : "memory");
        if constexpr (NR >= 2)
            asm volatile("vse32.v v24, (%0)" : : "r"(d_row_5 + 1 * stride_col) : "memory");
        if constexpr (NR >= 3)
            asm volatile("vse32.v v25, (%0)" : : "r"(d_row_5 + 2 * stride_col) : "memory");
    }
    if constexpr (MR >= 7) {
        float *d_row_6 = D + 6 * stride_row;
        if constexpr (NR >= 1)
            asm volatile("vse32.v v26, (%0)" : : "r"(d_row_6 + 0 * stride_col) : "memory");
        if constexpr (NR >= 2)
            asm volatile("vse32.v v27, (%0)" : : "r"(d_row_6 + 1 * stride_col) : "memory");
        if constexpr (NR >= 3)
            asm volatile("vse32.v v28, (%0)" : : "r"(d_row_6 + 2 * stride_col) : "memory");
    }
    if constexpr (MR >= 8) {
        float *d_row_7 = D + 7 * stride_row;
        if constexpr (NR >= 1)
            asm volatile("vse32.v v29, (%0)" : : "r"(d_row_7 + 0 * stride_col) : "memory");
        if constexpr (NR >= 2)
            asm volatile("vse32.v v30, (%0)" : : "r"(d_row_7 + 1 * stride_col) : "memory");
        if constexpr (NR >= 3)
            asm volatile("vse32.v v31, (%0)" : : "r"(d_row_7 + 2 * stride_col) : "memory");
    }
}

/// Fused load + vfmacc kernel (A*B) with column-first ordering and software pipelining.
/// B is loaded with vle32 (contiguous). Used for packed and non-packed B variants.
/// Computes current iteration's MR*NR vfmacc operations while prefetching
/// next iteration's A scalars (into ft0-ft7) and B vectors (into v0-v2).
/// Assumes ft0-ft7 and v0-v2 already hold the current iteration's data.
template <size_t MR, size_t NR>
static inline void tqt_fused_load_vfmacc_axb_kernel_gemm_f32_f32_8x3vl_rvv(const float *A_next,
                                                                           size_t a_stride_row,
                                                                           const float *B_next,
                                                                           size_t b_col_stride)
{
    ...
    if constexpr (MR >= 8)
        asm volatile("vfmacc.vf v29, ft7, v0" ::: "v29");
    // v0 freed → prefetch next B col 0
    ...
    if constexpr (NR >= 2) {
        ...
        if constexpr (MR >= 8)
            asm volatile("vfmacc.vf v30, ft7, v1" ::: "v30");
        // v1 freed → prefetch next B col 1
        asm volatile("vle32.v v1, (%0)" : : "r"(B_next + b_col_stride) : "v1", "memory");
    }

    // =================================================================
    // Last column: vfmacc interleaved with next iteration's A loads.
    // After each vfmacc using ftX, ftX is freed and reloaded for next K.
    // =================================================================
    if constexpr (NR == 3) {
        // Column 2: vfmacc with v2, interleaved with A prefetch
        if constexpr (MR >= 1) {
            asm volatile("vfmacc.vf v10, ft0, v2" ::: "v10");
        ...
        if constexpr (MR >= 5) {
            asm volatile("vfmacc.vf v22, ft4, v2" ::: "v22");
            asm volatile("flw ft4, 0(%0)" : : "r"(A_next + 4 * a_stride_row) : "ft4", "memory");
        }
        if constexpr (MR >= 6) {
            asm volatile("vfmacc.vf v25, ft5, v2" ::: "v25");
            asm volatile("flw ft5, 0(%0)" : : "r"(A_next + 5 * a_stride_row) : "ft5", "memory");
        }
        if constexpr (MR >= 7) {
            asm volatile("vfmacc.vf v28, ft6, v2" ::: "v28");
            asm volatile("flw ft6, 0(%0)" : : "r"(A_next + 6 * a_stride_row) : "ft6", "memory");
        }
        if constexpr (MR >= 8) {
            asm volatile("vfmacc.vf v31, ft7, v2" ::: "v31");
            asm volatile("flw ft7, 0(%0)" : : "r"(A_next + 7 * a_stride_row) : "ft7", "memory");
        }
        // v2 freed → prefetch next B col 2
        asm volatile("vle32.v v2, (%0)" : : "r"(B_next + 2 * b_col_stride) : "v2", "memory");

    } else {
        // NR=1 or NR=2: last vfmacc column already emitted above.
        // A prefetch: load sequentially (no vfmacc to interleave with)
        if constexpr (MR >= 1)
            asm volatile("flw ft0, 0(%0)" : : "r"(A_next + 0 * a_stride_row) : "ft0", "memory");
        if constexpr (MR >= 2)
            asm volatile("flw ft1, 0(%0)" : : "r"(A_next + 1 * a_stride_row) : "ft1", "memory");
...
/// B is loaded with vlse32 (strided). Used for transposed B variants.
/// b_stride_row is the row stride of transposed B matrix (= ldb).
template <size_t MR, size_t NR>
static inline void tqt_fused_load_vfmacc_axbt_kernel_gemm_f32_f32_8x3vl_rvv(
    size_t vl, const float *A_next, size_t a_stride_row, const float *B_next, size_t b_stride_row)
{
    ...
    if constexpr (MR >= 8)
        asm volatile("vfmacc.vf v29, ft7, v0" ::: "v29");
    // v0 freed → prefetch next B col 0 (strided)
    asm volatile("vlse32.v v0, (%0), %1"
                 :
    ...
    if constexpr (NR >= 2) {
        ...
        if constexpr (MR >= 8)
            asm volatile("vfmacc.vf v30, ft7, v1" ::: "v30");
        // v1 freed → prefetch next B col 1 (strided)
        asm volatile("vlse32.v v1, (%0), %1"
                     :
    ...
    if constexpr (NR == 3) {
        ...
        // v2 freed → prefetch next B col 2 (strided)
        asm volatile("vlse32.v v2, (%0), %1"
                     :
        ...
        // NR=1 or NR=2: A prefetch (sequential, no vfmacc to interleave with)
        if constexpr (MR >= 1)
            asm volatile("flw ft0, 0(%0)" : : "r"(A_next + 0 * a_stride_row) : "ft0", "memory");
        if constexpr (MR >= 2)
            asm volatile("flw ft1, 0(%0)" : : "r"(A_next + 1 * a_stride_row) : "ft1", "memory");
...
/// Used for the last K iteration when there is no next iteration to prefetch.
template <size_t MR, size_t NR>
static inline void tqt_epilogue_vfmacc_kernel_gemm_f32_f32_8x3vl_rvv()
{
    static_assert(MR >= 1 && MR <= 8, "MR must be in range [1, 8]");
    static_assert(NR >= 1 && NR <= 3, "NR must be in range [1, 3]");
