...
#include "tqt_gemm_1xnbias_clamp_f32_f32_f32t_2x8vl_rvv.h"
...
size_t tqt_get_m_step_gemm_1xnbias_clamp_f32_f32_f32t_2x8vl_rvv(void)
{
    return m_step;
}

size_t tqt_get_n_step_gemm_1xnbias_clamp_f32_f32_f32t_2x8vl_rvv(void)
{
    return n_step;
}

size_t tqt_get_a_offset_gemm_1xnbias_clamp_f32_f32_f32t_2x8vl_rvv(size_t m_idx, size_t k_idx,
                                                                  size_t lda)
{
...
size_t tqt_get_b_offset_gemm_1xnbias_clamp_f32_f32_f32t_2x8vl_rvv(size_t n_idx, size_t k_idx,
                                                                  size_t ldb)
{
...
size_t tqt_get_c_offset_gemm_1xnbias_clamp_f32_f32_f32t_2x8vl_rvv(size_t m_idx, size_t n_idx,
                                                                  size_t ldc)
{
    TQT_ASSUME(m_idx % m_step == 0);
    TQT_ASSUME(n_idx % n_step == 0);
...
size_t tqt_get_bias_offset_gemm_1xnbias_clamp_f32_f32_f32t_2x8vl_rvv(size_t n_idx)
{
    TQT_ASSUME(n_idx % n_step == 0);
    return n_idx * sizeof(float);
}

size_t tqt_get_d_offset_gemm_1xnbias_clamp_f32_f32_f32t_2x8vl_rvv(size_t m_idx, size_t n_idx,
                                                                  size_t ldd)
{
    TQT_ASSUME(m_idx % m_step == 0);
    TQT_ASSUME(n_idx % n_step == 0);
...
size_t tqt_get_d_size_gemm_1xnbias_clamp_f32_f32_f32t_2x8vl_rvv(size_t m, size_t n)
{
...
// Template-based GEMM kernel dispatcher for 2x8vl (LMUL=8) with transposed B.
// Handles arbitrary vl in [1, 8*vlmax] via RVV's variable-length vector semantics.
template <size_t MR>
                            ...
                            float clamp_min, float clamp_max, size_t vl)
{
    ...
    tqt_load_a_kernel_gemm_f32_f32_2x8vl_rvv<MR>(a_ptr, a_stride_row);
    tqt_load_bt_kernel_gemm_f32_f32_2x8vl_rvv(vl, b_ptr, ldb);
    a_ptr += 1;
    ...
    for (size_t k_idx = 1; k_idx < K; k_idx++) {
        tqt_fused_load_vfmacc_axbt_kernel_gemm_f32_f32_2x8vl_rvv<MR>(vl, a_ptr, a_stride_row, b_ptr,
                                                                     ldb);
        a_ptr += 1;
        b_ptr += 1;
    }
...
void tqt_run_gemm_1xnbias_clamp_f32_f32_f32t_2x8vl_rvv(size_t m, size_t n, size_t k, const void *A,
