...
#include "tqt_convcf_mx1bias_clamp_f16_f16p_f16p_8x3vl_rvv.h"
...
{
    return tqt_get_m_step_gemm_mx1bias_clamp_f16_f16p_f16p_8x3vl_rvv();
}

size_t tqt_get_n_step_convcf_mx1bias_clamp_f16_f16p_f16p_8x3vl_rvv(void)
{
    return tqt_get_n_step_gemm_mx1bias_clamp_f16_f16p_f16p_8x3vl_rvv();
}
...
size_t tqt_get_a_packed_offset_convcf_mx1bias_clamp_f16_f16p_f16p_8x3vl_rvv(size_t m_idx,
                                                                            ...
                                                                            size_t actual_m)
{
    return tqt_get_a_packed_offset_gemm_mx1bias_clamp_f16_f16p_f16p_8x3vl_rvv(m_idx, k_idx, lda,
                                                                              actual_m);
}
...
size_t tqt_get_c_offset_convcf_mx1bias_clamp_f16_f16p_f16p_8x3vl_rvv(size_t m_idx, size_t n_idx,
                                                                     size_t ldc)
{
    return tqt_get_c_offset_gemm_mx1bias_clamp_f16_f16p_f16p_8x3vl_rvv(m_idx, n_idx, ldc);
}

size_t tqt_get_bias_offset_convcf_mx1bias_clamp_f16_f16p_f16p_8x3vl_rvv(size_t m_idx)
{
    return tqt_get_bias_offset_gemm_mx1bias_clamp_f16_f16p_f16p_8x3vl_rvv(m_idx);
}

size_t tqt_get_d_offset_convcf_mx1bias_clamp_f16_f16p_f16p_8x3vl_rvv(size_t m_idx, size_t n_idx,
                                                                     size_t ldd)
{
    return tqt_get_d_offset_gemm_mx1bias_clamp_f16_f16p_f16p_8x3vl_rvv(m_idx, n_idx, ldd);
}

size_t tqt_get_d_size_convcf_mx1bias_clamp_f16_f16p_f16p_8x3vl_rvv(size_t m, size_t n)
{
    return tqt_get_d_size_gemm_mx1bias_clamp_f16_f16p_f16p_8x3vl_rvv(m, n);
}

size_t tqt_get_a_packed_size_convcf_mx1bias_clamp_f16_f16p_f16p_8x3vl_rvv(size_t m, size_t k)
{
    return tqt_get_a_packed_size_gemm_mx1bias_clamp_f16_f16p_f16p_8x3vl_rvv(m, k);
}

size_t tqt_get_b_im2colpack_size_convcf_mx1bias_clamp_f16_f16p_f16p_8x3vl_rvv(size_t n, size_t k)
{
    return tqt_get_b_packed_size_gemm_mx1bias_clamp_f16_f16p_f16p_8x3vl_rvv(n, k);
}
...
size_t tqt_get_b_im2colpack_offset_convcf_mx1bias_clamp_f16_f16p_f16p_8x3vl_rvv(size_t n_idx,
                                                                                ...
                                                                                size_t actual_n)
{
    return tqt_get_b_packed_offset_gemm_mx1bias_clamp_f16_f16p_f16p_8x3vl_rvv(n_idx, k_idx, ldb,
                                                                              actual_n);
}
...
{
    tqt_run_a_pack_gemm_mx1bias_clamp_f16_f16p_f16p_8x3vl_rvv(m, k, lda, lda_packed, k_idx, A,
                                                              A_packed);
}
...
static void b_im2colpack_cf_fill(float16_t *dst, size_t sub_n, size_t col_start,
                                 const float16_t *input_group, size_t IC_per_group,
        ...
        for (size_t kd = 0; kd < KD; ++kd) {
            for (size_t kh = 0; kh < KH; ++kh) {
                for (size_t kw = 0; kw < KW; ++kw) {
                    float16_t *row_dst = dst + k_row * sub_n;
                    ...
                    while (remaining > 0) {
                        ...
                        if (id_in < 0 || id_in >= (int)ID || ih_in < 0 || ih_in >= (int)IH) {
                            memset(row_dst + wp, 0, cnt * sizeof(float16_t));
                        } else {
                            ...
                            if (SW == 1 && DW == 1) {
                                ...
                                size_t vc = cnt - pl - pr;
                                if (pl)
                                    memset(row_dst + wp, 0, pl * sizeof(float16_t));
                                if (vc) {
                                    const float16_t *s =
                                        input_group + ic * ID * IH * IW + (size_t)id_in * IH * IW +
                                        (size_t)ih_in * IW + (size_t)(iw_s + (int)pl);
                                    float16_t *d = row_dst + wp + pl;
                                    size_t r = vc;
                                    while (r > 0) {
                                        size_t vl = __riscv_vsetvl_e16m1(r);
                                        __riscv_vse16_v_f16m1(d, __riscv_vle16_v_f16m1(s, vl), vl);
                                        s += vl;
                                        d += vl;
                                        r -= vl;
                                    }
                                ...
                                if (pr)
                                    memset(row_dst + wp + cnt - pr, 0, pr * sizeof(float16_t));
                            } else {
    ...
    const tqt_conv_params *params)
{
    ...
    const float16_t *input_group = input + group_idx * IC_per_group * params->input_shape[0] *
                                               params->input_shape[1] * params->input_shape[2];
    float16_t *packed = (float16_t *)B_packed;

    const size_t vlmax = csrr_vlenb() / sizeof(float16_t);
...
{
    tqt_run_gemm_mx1bias_clamp_f16_f16p_f16p_8x3vl_rvv(m, n, k, A_packed, lda, k_idx_a, B_packed,
